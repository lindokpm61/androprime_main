...
- **The blocking guard's real scope, read from the source 2026-07-31 (Observation 74).** Its `COPY_ROOTS` are `09_website-app/frontend/{content,app,components}/` plus **the whole of `06_marketing/`, `07_sales/` and `08_customer-journey/`**, on `.md`, `.mdx`, `.html`, `.txt`, `.tsx`. That is far wider than "customer-facing copy": it covers internal SEO strategy, calendars, coverage rules, master plans and SOPs that no customer will ever read. It is also **whole-file, not diff-scoped** — it re-reads the saved file and blocks on ANY em dash in it, including hundreds you did not introduce.
- **What that means in practice, and it matters on any cross-cutting edit.** A single-line edit to a legacy dash-heavy doc will block and report every pre-existing em dash in the file. **Do not rewrite unrelated em dashes to clear it**: that produces a large unreviewed diff nobody asked for, in a doc that is not customer copy. Fix only what you introduced, state plainly in the report that the guard fired on pre-existing content you deliberately left, and move on. During the CA-028 sweep on 2026-07-31 it fired about twenty times across seventeen files, up to 75 pre-existing hits in one file, none of them introduced by the edit. Being trained to dismiss a blocking guard is the actual risk here; say out loud each time that you dismissed it and why. (The fix, scoping the guard to `tool_input.new_string`, is logged as Observation 91 and is a code change, not a wrap chore.)
