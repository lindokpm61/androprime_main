...
- **Two em-dash hooks exist, with different severity — do not model them as one** (both gitignored/local, `.claude/hooks/`): (a) a **non-blocking** PostToolUse warning that fires on internal docs and only reports a count (it never sweeps pre-existing content, so old em dashes persist until a manual sweep); (b) a **BLOCKING** guard scoped to customer-facing copy paths (e.g. `06_marketing/content/`, and it has also fired on `07_sales/funnel/` — the glob may be broader than intended) that refuses the write until the em dash is removed. When a save is blocked, fix the flagged line and re-save; do not assume "non-blocking" and move on. (Observations 14 + 15, 2026-07-25.)
