...
- **"No MOQ" positioning** — claims attractive to startup volumes (needs verification for custom formulations specifically)
- **120+ stock formulas** — possible alternative path: start with closest stock formula match, customise later when volumes scale
- **Specific collagen private-label range** — useful for our Joint & Recovery Collagen, possibly reducing custom formulation effort and cost
- **Dropshipping option** — would meaningfully reduce launch fulfilment complexity (no inventory storage burden, especially while Gate 0A volumes are uncertain)
- **GMP certified** — meets baseline regulatory requirement
- **UK-based** — geographic alignment

---

## What We Want From This Conversation

1. **MOQ verification** — does the "no MOQ" claim hold for **custom** formulations, or only stock formulas?
2. **Stock formula option** — is there a Daily Stack-equivalent in their 120+ catalogue we could trial first?
3. **Their collagen private-label range** — does the existing formulation match our spec or how close can we get?
4. **Dropshipping economics** — per-unit cost differential vs traditional bulk-then-ship model
5. **3-tier pricing quote:** 1,500 / 5,000 / 25,000 unit volumes for each product
6. **Active B12 dose flexibility** — quote both 1,000mcg and 5,000mcg Methylcobalamin

---

## Detailed Question List (for follow-up call after first reply)

**Critical first-call question:**
1. **Does your "no MOQ" claim apply to fully custom formulations, or only to stock private-label products?** This is the make-or-break question. If only stock formulas, the conversation pivots to "what's closest in your existing range?"

**Daily Stack V7.2 — custom path:**
2. If custom is supported with low/no MOQ: same questions as Vita / Synergy on capsule sizing, pure Methylcobalamin sourcing, KSM-66 sourcing
3. Pricing per unit at 1,500 / 5,000 / 25,000 volumes for the custom Daily Stack formulation

**Daily Stack — stock formula path:**
4. Closest stock formula match? (Looking for: Zinc + D3 + B12 combo with Ashwagandha if possible)
5. Customisation options on stock formulas — can specific ingredients be swapped or upgraded (e.g. swapping cyanocobalamin for Methylcobalamin if their stock uses the cheaper form)?
6. Pricing per unit on stock vs custom path

**Joint & Recovery Collagen:**
7. Their existing collagen private-label range — formulations available?
8. How close to our spec (Hydrolysed Bovine 10g, UC-II 40mg, Vitamin C 80mg, MSM 500mg, Hyaluronic Acid 5mg)?
9. Customisation pathway from stock collagen to our exact spec
10. Sachet filling capability on collagen specifically — included in their range or separate?

**Dropshipping option:**
11. Mechanics — do you fulfil direct to customer using our branding, or is the brand secondary?
12. Per-unit dropship pricing vs bulk-purchase pricing
13. Order capacity — do you handle 20 orders/day at launch + scaling to 100/day?
14. Customer service handover — what happens when a customer queries delivery? Does the query land with you or with us?
15. Returns / replacement handling

**Both products / commercial:**
16. Payment terms (especially relevant for dropship — do we pre-fund or pay-per-order?)
17. Stability testing for V7.2 reformulated Daily Stack — supported in-house or external?
18. Lead time from formulation lock to first batch (or first dropship-ready order)

---

## Negotiation Levers (per outreach tracker)

**Stock-formula entry strategy:**
> "If your custom MOQ is high but stock formulas have low/no MOQ, we'd be open to launching with the closest stock match while we accumulate volume to justify a custom run. What's the best stock fit for a Zinc + D3 + Methylcobalamin + Ashwagandha capsule?"

**Dropship trial:**
> "Dropshipping is attractive for Gate 0A volumes. We'd want to start dropship to derisk inventory, then transition to bulk purchase when we hit ~5,000 units/month. Is that operationally feasible at your end?"

**Volume commitment for custom:**
> "If your custom MOQ is the blocker, we'd commit to 5,000 units/month for first 6 months in exchange for low first-batch MOQ. Can you accommodate?"

---

## Draft Outreach Email

**To:** via nutribl.com contact form
**Subject:** UK men's health brand — custom formulation + dropship enquiry

> Hi,
>
> I'm Keith Antony, founder of Andro Prime — a UK men's health brand launching at-home blood test kits and a results-driven supplement subscription range. We're preparing our Phase 0 launch and identifying our manufacturing partner for two supplement products.
>
> Two reasons I'm contacting Nutribl specifically:
>
> 1. Your "no MOQ" positioning is unusual for custom formulations and would be a significant fit for our launch volumes (~1,500 units first batch). I'd like to understand whether that applies to fully custom formulations or only to your stock private-label range.
>
> 2. Your existing collagen private-label range could be a fast path for our Joint & Recovery Collagen if there's a close formulation match. Would save us a custom development cycle.
>
...
> - Can your collagen private-label range be customised to our exact spec, or are we looking at a fully custom formulation?
> - Is dropshipping operationally viable from Day 1 given our ~20-50 orders/day projected at launch, scaling to 100+/day by Month 6?
>
...
## Decision Criteria for Nutribl Specifically

Nutribl wins this slot if:
- **"No MOQ" applies to custom formulations OR a close stock formula exists** for Daily Stack
- **Collagen private-label range can be customised to within ~80% of our spec** (acceptable formulation drift on sub-ingredients)
- **Dropship economics work** at Gate 0A volumes (per-order cost ≤ £4 above bulk per-unit cost)
- **Methylcobalamin** is available (not just cyanocobalamin) — non-negotiable for "Active B12" branding
- **First-call responsiveness within 5 business days**

The dropship play is unique to Nutribl on this shortlist. If feasible, it materially reduces launch cash exposure even compared to the hybrid free-kit + 10% discount model. Worth weighting heavily.

---

## Follow-up Plan

- **Day 0:** Send email above (form submission)
- **Day 5:** If no reply, attempt direct contact via any phone number on their site
- **Day 10:** If still no reply, deprioritise — they market as accessible, slow response would be a signal
...
- `2026-MM-DD-discovery-call-notes.md`
- `pricing-quote.md` (with separate columns for stock-formula vs custom vs dropship pricing)
- `dropship-evaluation.md` (if dropship is offered)
