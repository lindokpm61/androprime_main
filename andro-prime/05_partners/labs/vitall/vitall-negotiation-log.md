...
- **Packaging: REPLY DRAFTED 2026-08-23, NOT SENT** (Gmail draft `r4593140092559900756`, threaded into `19e80ec01a3f40e6`; doc `correspondence/2026-08-23-keith-sleeve-cutter-guide-and-kitting.md`). **His 3 June reply answered everything and sat unanswered for 11 weeks** — the thread read as closed because the last message was inbound and nothing was owed on his side. Leads on the **cutter guide**, which closes aperture size, corner radius, fold position and tolerance in one artefact. Key new fact driving it: Keith checked a physical kit and **the details panel is DIE-CUT out of the sleeve**, with Vitall's label on the box beneath showing through — so a die already exists and matching it exactly is what avoids tooling cost. Also asks: die availability, sleeve-on-box play, label dimensions, print-or-supply, 500-per-design-or-total, setup time, delivery address, whether the in-house kitting solution has landed, plus the IFU variant, the request-form logo, and the **CA-019 per-kit collection protocol** gate. No artwork attached, deliberately: four of the answers could change the file.
