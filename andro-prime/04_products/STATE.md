...
rules; `normal-vitamin-d` sits there and has no kind at all. **Anything deriving a kind from a
bucket heading gets those three wrong, silently.** The new file carries the build checklist and
names the three blockers: design the `seasonal` kind, add the two missing rows, re-home the
anti-upsell guard.
...
🔴 **Q5 = A preserves a ruling the design cannot store.** "Retest heading into autumn or winter" is
neither `{ days }` nor `{ fromMonths, toMonths }`. **`RETEST_CADENCE` needs a fifth kind** before
`normal-vitamin-d` can be filled at all, or the cell falls back to `clinician-led` and shows a man
no date on a perfectly normal result. A gap her answer exposed in the design, not a defect in it.
...
**The one coverage item neither round closed:** two signed states still have **no row** in the
table (T > 29 nmol/L, vitamin D > 250 nmol/L), because that is a missing row rather than an
unanswered question.

⚠ **In the other direction, she signed two states the table has no rows for**: **T > 29 nmol/L**
and **vitamin D > 250 nmol/L**, both `clinician-led`. They exist in `thresholds.md` (added
2026-08-07) but were never carried into the pack. **Add both rows before building**, or the
`Record` cannot be exhaustive, which is the whole reason that shape was chosen.
