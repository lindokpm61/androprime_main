# Andro Prime — Non-Regulated Tier V7.2 Financial Model
## Organic-First | Zero Paid Media | Premium Positioning
### April 2026 (pricing reconciled to v2.2 canonical 2026-05-08; affiliate stack reconciled to v2.3 2026-05-09)

**Supersedes:** V7.1 financial projections (unit economics and forecast sections only)
**Status:** Unit economics reconciled to v2.2 canonical pricing (£99/£119/£179). Section 4 forecast tables and Section 6 KPIs are STALE pending spreadsheet rerun against the canonical prices.
**Owner:** Keith Antony
**Spreadsheet:** `01_strategy/financial-model/andro-prime-phase0-v72.xlsx`

---

> **PRICING RECONCILIATION NOTE 2026-05-08:** This document originally used £89/£99/£149 retail pricing — a transitional stage that has been superseded by the v2.2 canonical pricing of **£99/£119/£179**, as encoded in [`product-catalogue-v7-1.md`](product-catalogue-v7-1.md), [`phase0-marketing-plan.md`](../../06_marketing/master-plan/phase0-marketing-plan.md) v2.2, and [`phase0-financial-model-v1.md`](../../01_strategy/phase0-financial-model-v1.md). Per Keith 2026-05-08, v2.2 is the authoritative pricing schema for all downstream copy, financial modelling, and partner programmes (PT-coded prices: £89.10 / £107.10 / £161.10 = 10% off canonical retail). Section 2.1 unit economics below are reconciled. **Sections 4.2, 4.3, 4.4, and 6 are pre-reconciliation and need a spreadsheet rerun against £99/£119/£179 — do not cite their forecast figures without first reconciling against the spreadsheet.** Surfaced as a data-hygiene blocker by the kit-strategy decision brief 2026-05-08; this note resolves the document-side reconciliation.

---

## 1. What Changed from V7.1

V7.1 used competitor retail pricing as a proxy for COGS. Actual Vitall lab quotes revealed competitor pricing is loss-leader pricing subsidised by clinical revenue. This update reflects real lab costs, repriced retail to maintain 30–40% gross margin, and restructures the operating model around zero paid media with organic/affiliate-only acquisition.

Key changes:

- Kit COGS updated to actual Vitall finger-prick quotes (3–4× higher than V7.1 estimates)
- Retail prices repositioned premium: **£99 / £119 / £179** (canonical v2.2; replaces the transitional £89/£99/£149 stage and the original V7.1 £29/£44/£69)
- Kit 2 panel: Magnesium removed, B12 confirmed (Vitall availability)
- Paid media budget: £0 (from £18,000 over 6 months)
- Affiliate model: **v2.3 right-sized stack — £15 flat per kit + £10 Kit 3 upsell + £10 supplement-conversion + Silver £200 / Gold £400 PT graduation bonuses (one-off, PT only)**. Replaces both the legacy 20%/12.5% commission models and the earlier v2.2 stack (which included +£10 first-month, +£20 Kit 3 uplift, monthly tier retention, PT-refers-PT, and £250/mo flagship retainer — all dropped 2026-05-08).
- Customer discount via PT/influencer codes: **10% off retail** (PT-coded prices £89.10 / £107.10 / £161.10) per v2.2.
- Affiliate sales mix: **50/50 direct vs PT-coded** per v2.3 (carried over from v2.2 hybrid model; replaces legacy 20% / 12.5% commission assumptions)
- Launch capital separated into two tranches: kits-only launch (£3,000) and supplement capital (£9,000 at Gate 0A). ⛔ **The £9,000 Gate-0A figure is Option B (both SKUs) and is retired as the 0A cap, 2026-07-09.** Restated Gate 0A caps first-run exposure at the **phased ~£5,950 (Option A, Daily Stack only)** — see `01_strategy/CONTEXT.md` → Gates Reference, and `01_strategy/financial-model/phase0-financial-model-v1.md` §5, which recommends Option A. Capital is now **fronted** rather than drawn from accumulated cash (Tier-2 founder bet), so the "no external funding required" framing at line 126 no longer describes the live plan.

---

## 2. Unit Economics

### 2.1 Kits (Vitall Finger-Prick)

*Reconciled to v2.2 canonical pricing 2026-05-08.*

| Product | COGS | Retail | Gross Margin | Margin % |
|---------|------|--------|-------------|----------|
| Kit 1 — Testosterone Health Check | £58.50 | £99 | £40.50 | 40.9% |
| Kit 2 — Energy & Recovery Check | £63.00 | £119 | £56.00 | 47.1% |
| Kit 3 — Hormone & Recovery Check | £98.00 | £179 | £81.00 | 45.3% |

After payment processing (2.5% Stripe) and v2.3 affiliate model (£15 flat fee + Kit 3 £10 bonus, 10% customer discount on PT/affiliate-coded sales, 50/50 sales mix):

| Metric | Kit 1 | Kit 2 | Kit 3 |
|--------|-------|-------|-------|
| Direct sale net (full £99/£119/£179 retail, after 2.5% Stripe) | £38.02 | £53.03 | £76.53 |
| Affiliate sale net (£89.10/£107.10/£161.10 retail with 10% PT discount, after £15 flat + Kit 3 £10 bonus + 2.5% Stripe) | £13.37 | £26.42 | £34.07 |
| **Blended net contribution (50/50 v2.2 hybrid)** | **£25.70** | **£39.72** | **£55.30** |

Reconciles to v2.3 affiliate stack. Excludes supplement-conversion bonus (+£10) — paid only on conversion events, not per kit. The v2.2 first-month +£10 bonus has been dropped in v2.3.

### 2.2 Supplements (Market Research Estimates — Pending Manufacturer Quotes)

*Daily Stack COGS reconciled 2026-05-08 to £12 (midpoint of the £11–13 range per [`daily-stack.md`](../supplements/daily-stack.md) §"Manufacturer Brief Notes" line 90). Complete Stack bundle COGS recalculated.*

| Product | Sub Price /mo | COGS (Est.) | Gross Margin | Margin % |
|---------|--------------|-------------|-------------|----------|
| Daily Stack | £34.95 | £12.00 | £22.95 | 65.7% |
| Joint & Recovery Collagen | £29.95 | £13.00 | £16.95 | 56.6% |
| Complete Stack (Bundle) | £54.95 | ~£25.00 | ~£29.95 | 54.5% |

---

## 3. Monthly Breakeven

| Metric | Value |
|--------|-------|
| Monthly fixed overhead | £593 |
| Blended kit margin (avg across 3 kits) | ~£33.55 |
| **Kits needed per month to cover overhead** | **18** |
| **Kits needed per week** | **5** |

This is the minimum viable throughput. Anything above 18 kits/month generates net positive cash from operations.

---

## 4. Six-Month Forecast — Planning Case

> **STALE 2026-05-08:** The revenue, profitability, and cash-position tables in §§4.2–4.4 below are calculated against the transitional £89/£99/£149 retail pricing and need a spreadsheet rerun against the v2.2 canonical £99/£119/£179. Volume assumptions in §4.1 are unaffected. **Do not cite figures from §§4.2–4.4 in any new strategic document until the spreadsheet rerun is complete.** The kit-strategy decision brief 2026-05-08 uses §2.1 (now reconciled) plus the Phase 0 marketing plan v2.2 figures, which are correct.

### 4.1 Volume Assumptions

Organic-only acquisition. **Option 4 (locked 2026-05-08): Kit 1 and Kit 2 promoted at equal pace through Phase 0** — earlier "Kit 2 leads" framing superseded. Kit 3 staged from Month 2. Pre-launch waitlist and affiliate network drive Month 1 volume. (Volume assumptions in the table below are unaffected by the Option 4 reframe; the equal-pacing change is in marketing emphasis, not kit mix targets — pending spreadsheet rerun against canonical Option 4 inputs.)

| Month | Kit 1 | Kit 2 | Kit 3 | Total |
|-------|-------|-------|-------|-------|
| M1 | 5 | 12 | 0 | 17 |
| M2 | 8 | 16 | 3 | 27 |
| M3 | 10 | 22 | 5 | 37 |
| M4 | 12 | 28 | 8 | 48 |
| M5 | 15 | 32 | 10 | 57 |
| M6 | 18 | 38 | 12 | 68 |
| **Total** | **68** | **148** | **38** | **254** |

### 4.2 Revenue

| Stream | M1 | M2 | M3 | M4 | M5 | M6 | 6-Mo Total |
|--------|---:|---:|---:|---:|---:|---:|---:|
| Kit Revenue | £1,633 | £2,743 | £3,813 | £5,032 | £5,993 | £7,152 | £26,366 |
| Supplement Revenue | £34 | £104 | £209 | £349 | £524 | £733 | £1,957 |
| **Total Revenue** | **£1,667** | **£2,847** | **£4,022** | **£5,381** | **£6,517** | **£7,885** | **£28,323** |

### 4.3 Profitability

| Metric | M1 | M2 | M3 | M4 | M5 | M6 | 6-Mo Total |
|--------|---:|---:|---:|---:|---:|---:|---:|
| Gross Profit | £679 | £1,182 | £1,696 | £2,281 | £2,793 | £3,397 | £12,032 |
| Gross Margin | 39.0% | 39.5% | 39.9% | 40.2% | 40.5% | 40.8% | 40.3% |
| Total OpEx | £677 | £736 | £794 | £860 | £915 | £980 | £4,964 |
| **Net Contribution** | **£2** | **£446** | **£902** | **£1,420** | **£1,878** | **£2,417** | **£7,067** |
| Net Margin | 0.1% | 14.9% | 21.2% | 25.0% | 27.3% | 29.0% | 23.6% |

### 4.4 Cash Position

No launch capital required (Vitall white-label). Supplement capital deferred until self-funded from kit profits.

| Item | M1 | M2 | M3 | M4 | M5 | M6 |
|------|---:|---:|---:|---:|---:|---:|
| Launch Capital | £0 | — | — | — | — | — |
| Monthly Net | £2 | £446 | £902 | £1,420 | £1,878 | £2,417 |
| **Cumulative Cash** | **£2** | **£448** | **£1,351** | **£2,771** | **£4,650** | **£7,068** |

Cash is positive from Month 1 and accumulates throughout. Supplement manufacturing capital (£5,950 phased or £9,000 full) is drawn from this cash reserve when the threshold is reached — no external funding required.

---

## 5. Capital Structure

### Kit Launch: £0

Vitall is white-label drop-ship. All lab, collection kit, postage, and packaging costs are built into the per-kit COGS (£58.50 / £63.00 / £98.00). No inventory held. No setup fee. No upfront platform cost. The business launches with zero capital outlay.

### Supplement Manufacturing: Self-Funded from Kit Revenue

Supplement capital is only committed after Gate 0A confirms kit demand, and only when accumulated kit profits cover the cost. No personal investment required — the kit business funds the supplement business.

**Full launch (both SKUs):**

| Item | Amount |
|------|--------|
| Daily Stack MOQ (500–1,000 units) | £4,000 |
| Collagen MOQ (500 units) | £1,500 |
| Label design + compliance review (2 SKUs) | £2,000 |
| Stability testing (2 SKUs) | £1,500 |
| **Total** | **£9,000** |

At planning case volumes, cumulative net cash reaches £7,068 by Month 6. Full supplement capital of £9,000 is reachable by approximately Month 7. Add 5–6 weeks manufacturing lead time — supplements available for sale by approximately Month 9.

**Phased option (Daily Stack only first):**

| Item | Amount |
|------|--------|
| Daily Stack MOQ (500 units) | £4,000 |
| Label design + compliance (1 SKU) | £1,200 |
| Stability testing (1 SKU) | £750 |
| **Total** | **£5,950** |

Phased capital of £5,950 is reachable by approximately Month 5. Supplements available by Month 7. Collagen added later once Daily Stack proves conversion. This accelerates the subscription revenue engine by 2 months versus the full launch option.

---

## 6. Key Metrics — End of Month 6

> **STALE 2026-05-08:** Below figures derive from §§4.2–4.4 calculated against £89/£99/£149. Pending spreadsheet rerun against v2.2 canonical £99/£119/£179.

| Metric | Value |
|--------|-------|
| Total kits sold | 254 |
| Active supplement subscribers | 21 |
| Supplement MRR | £734 |
| Founding-member list opt-ins (cumulative, non-cash) | 21 |
| 6-month total revenue | £28,323 |
| 6-month net contribution | £7,067 |
| 6-month net margin | 23.6% |
| Cumulative cash position | +£7,068 |

---

## 7. Sensitivities and Risks

### Month 1 is razor-thin
Net contribution of £2 requires exactly 17 kits sold with no paid acquisition. If Month 1 delivers 15 kits instead of 17, the month is net-negative by ~£65. The pre-launch waitlist is critical — without 200+ warm contacts at launch, Month 1 may not hit breakeven.

### Supplement COGS are pending manufacturer confirmation
Daily Stack at **£12 COGS** is the reconciled midpoint (2026-05-08 reconciliation; range £11–£13 per `daily-stack.md`). At £12, supplement margin is 65.7% on the £34.95 sub price. If actual COGS comes in at £14 (above current research range), supplement margin drops to ~60%. This doesn't break the model — supplements are a small revenue share in the first 6 months — but it matters increasingly from Month 6 onward as MRR grows. (Stale £15 / £17.50 figures from V7.1 retired in the 2026-05-08 reconciliation.)

### v2.3 affiliate stack is untested at-scale
The v2.3 right-sized stack (£15 flat per kit + £10 Kit 3 upsell + £10 supplement-conversion + Silver £200 / Gold £400 PT graduation bonuses) replaces the legacy 20% / 12.5% percentage-commission assumptions. Per-PT 6-month earning range: ~£13–16.5k mid-case (see `project_pt_programme_rightsizing` memory and `06_marketing/affiliates/commission-structure.md` v2.3). If PT recruitment falls below the 50/50 mix assumption, blended margin shifts toward the direct-sale figure (favourable). If PTs demand percentage-based comp instead, retail price or mix needs revisiting.

### Conversion rate is the make-or-break assumption
At 12% kit-to-supplement conversion, subscriber growth is slow (21 active by Month 6). At 18%, it doubles. The results report quality and supplement recommendation experience determine this number. It cannot be assumed — it must be measured from first kit results delivery.

---

**Last updated:** April 2026 (V7.2 unit economics) / 2026-05-08 (full v2.2 commercial-structure reconciliation: pricing £99/£119/£179, 50/50 sales mix, Daily Stack COGS £12 midpoint) / 2026-05-09 (v2.3 affiliate stack reconciliation: £15 flat + £10 Kit 3 + £10 supplement-conversion + Silver £200 / Gold £400 PT graduation; first-month/£20 Kit 3 uplift/monthly retention/PT-refers-PT/flagship retainer all dropped; FM £75 deposit shelved — Deposit Revenue row removed from §4.2; §§4.2–4.4 and §6 remain STALE pending spreadsheet rerun against these canonical inputs)
**Owner:** Keith Antony
