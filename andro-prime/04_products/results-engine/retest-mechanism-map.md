...
| 6 | ~~The symptom overlay red-flag line (that pack's Q4b)~~ ✅ **SIGNED 2026-09-15 (Q5 = A), verbatim as drafted:** chest pain, breathlessness at rest or on light exertion, unexplained weight loss, blood in stool or urine, a new lump, fainting, or any symptom new and getting worse week on week. ~~⚠ **It is on-screen copy for a symptomatic man and owes a compliance pre-flight before it ships**~~ ✅ **PRE-FLIGHTED AND APPROVED 2026-09-16 as CA-048**, assembled into the full escalation stack (999 block, then this list, then the panel suggestion) and signed by both Ewa and Keith. ✅ **AND BUILT 2026-09-16, GATED OFF.** All four build conditions are discharged: the NHS link and the alert container by ruling, the machine-read surface sweep by checking, and the render obligation by rendering at 390px in both themes. 🔴 **It reaches nobody, and the blocker is the TRIGGER rather than the screen** — nothing writes the symptom answer it keys off, so building that capture is separate work and a compliance question of its own | ~~Pre-flight the copy~~ **Closed** |
| 7 | 3d: ✅ **the second half is DECIDED AND BUILT 2026-09-13** — Keith: the fix belongs in **checkout, not the nightly job**. Branches 1 and 3 shipped behind `MEMBERSHIP_ENABLED`, 62 new assertions, and the clock reset is clamped so it can never hasten a retest. **What remains is which result states may pull the date forward**, section 7's proposal, and it is the whole of what is left of 3d. ✅ **RULED 2026-09-16 (CA-049 Q4 = B): any result with a flagged marker lets him pull the date forward to 3 months.** ⚠ **Unbuilt, and it touches dispatch**, so it needs the same clamp the membership clock reset already has, plus its own verification. Earlier: ✅ **ROUTED 2026-09-16:** question 4 of the Gmail draft `r7711330077156455653` to `ewalindo@live.co.uk`, packet tracked at ClickUp `869f2k7he`. ✅ **SENT AND ANSWERED 2026-09-16 02:03 UTC**, five letters against five questions (`1: A 2: A 3: B 4: B 5: A`), record CA-049. ⚠ **Correcting this row's own earlier line, which still said NOT SENT after the answer had been recorded two sentences above it.** Framed against her own 3-month interval rather than as an open number, and ⚠ **blast radius is marked on it: pulling a date forward dispatches a physical kit sooner, so it changes what arrives at his door and when** | Ewa on the intervals |
...
| 9 | 3f: ✅ **SCOPED AND SOCKETED 2026-09-13.** 🟡 **HALF-ANSWERED 2026-09-15, and it went the cheap way this entry predicted.** The GP-routed 10 are ruled: Ewa's Q3 = A is *"no Andro Prime retest interval at all, the GP directs the timing, and our card shows the referral rather than a retest date"* — which is exactly the *"his GP decides the interval, not us"* answer flagged here as cheaper than a date. The flagged 10 have an **interval** (3 months, Q2 = A) but **not a sentence**. ⚠ **A ruling on the interval is not the wording.** `retestGuidance.ts` classes both cohorts `owed`, meaning a sentence with no link and no price. ✅ **THE SUBSTANCE IS RULED 2026-09-16 (CA-049 Q3 = B): the flagged ten say *"your GP decides when to repeat this, not us"*, the same answer as the GP-routed ten.** She declined both "one sentence covering all ten" and "nothing at all". 🔴 **The WORDING is still owed**: it is new copy for ten cards and owes its own pre-flight. ⚠ **And the ruling put two live cards in breach** — `shbg-low` and `shbg-high` were selling "Retest in 6-12 months" — **corrected the same day** in `classifier.ts`, guarded from both directions. The 3-month interval survives as a scheduling value and **may not be stated on the card**. 🔴 **The email half is UNCHANGED and still a hard stop**: she did not rule on campaign 23's audience, CA-022 is still scoped to all-clear buyers, and campaign 23 is still `running` . ✅ **ROUTED 2026-09-16:** question 3 of the Gmail draft `r7711330077156455653` to `ewalindo@live.co.uk`, packet tracked at ClickUp `869f2k7he`. ✅ **SENT AND ANSWERED 2026-09-16 02:03 UTC**, five letters against five questions (`1: A 2: A 3: B 4: B 5: A`), record CA-049. ⚠ **Correcting this row's own earlier line, which still said NOT SENT after the answer had been recorded two sentences above it.** The ten states are enumerated in the email by description rather than by identifier, because she rules on results and not on row ids. ⚠ **The EMAIL HALF IS NOT IN THIS PACKET and is unchanged**: campaign 23 is still `running`, CA-022 is still scoped to all-clear buyers, and that audience question is a separate ask | Ewa to word; the interval is no longer the blocker, the sentence is |
