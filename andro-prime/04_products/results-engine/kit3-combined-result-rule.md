...
| 2 | Total T < 12 nmol/L | **Founding-member list** | **None** (FM list is the sole focus; do not split attention) | tiers 3–4 |
...
Notes:
- Tier 1 is **per-marker** and coexists with Tier 2. A Kit 3 man can have low T (Tier 2 sets the report headline + primary CTA) **and** Ferritin < 30 (that specific marker card still shows the GP-referral prompt). The GP prompt is never suppressed and never converted into a supplement, waitlist, or founding-member CTA.
- Tier 2 is **absolute**: when T < 12, the primary CTA is the founding-member list regardless of how many energy markers are also out of range. The "Complete Men's Stack" bundle is **not** offered as the primary CTA in this scenario. In v1 the Daily Stack secondary is dropped entirely — the FM list is the focus. The biomarker education for energy markers stays on each marker's own card.
...
2. Result: `deficiencyCount` is computed on energy markers only; the `low-testosterone` branch is always reached and, in v1, returns `primaryCta: foundingMember` with **no** secondary CTA (per §3). In v2 it returns `primaryCta: foundingMember`, `secondaryCta: dailyStackZinc`.
...
4. Add a regression fixture: Kit 3, T = 9 nmol/L + Vitamin D < 50 + Active B12 < 37.5. Expected (v1): testosterone card `primaryCta = founding-member-list`, no secondary CTA; energy-marker cards show their own waitlist CTAs; Complete Men's Stack does **not** appear as any primary CTA. (Add to `lib/results/fixtures/`.)
