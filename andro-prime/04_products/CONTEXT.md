...
```text
04_products/
├── CONTEXT.md                                 ← This file: durable rules, routing, source-of-truth pointers
├── STATE.md                                   ← Volatile status: DRAFT specs, proposed formulation changes, open decisions, sign-offs owed
...
│   └── non-regulated-tier-v72-financials.md   ← V7.2 financial model for the non-regulated tier
├── kits/
...
│   ├── kit-3-plus.md                          ← Kit 3 Plus spec: metabolic stack add-on (HbA1c, Fasting Insulin, ApoB, Homocysteine, lipids); deferred post-launch
│   ├── kit-4-metabolic-health-check.md        ← Superseded standalone metabolic kit (folded into Kit 3 Plus)
│   ├── kit-1-launch-guide.md                  ← Lab negotiations, Supabase schema, results report design
│   ├── liver-health-opportunity.md            ← Liver Health Check opportunity brief (fold into Kit 3 Plus or run standalone)
│   └── reverse-engineered-kit-analysis.md     ← Reverse-engineered kit / panel analysis
├── supplements/
...
│   ├── biomarker-supplement-loops.md          ← Biomarker → EFSA-supplement loop analysis (Phase 0 self-sustaining, no CQC dependency)
│   ├── omega-3-loop-spec.md                   ← DRAFT: Omega-3 Index → Omega-3 (EPA/DHA) test + supplement loop spec
│   └── supplement-demand-analysis.md          ← Supplement demand analysis
└── results-engine/
    ...
    ├── biomarker-copy.md                      ← LIVE source of per-biomarker result-card copy (until dashboard-copy.md is written)
    ├── kit3-combined-result-rule.md           ← Kit 3 combined-result precedence rule
    ├── 2026-06-04-low-t-routing-decision.md   ← Low-T (T<12) → GP-referral routing decision (Ewa CA-014), live in classifier.ts
    ├── 2026-06-04-low-t-routing-decision-sweep-report-2026-07-07.md ← Decision-sweep report for the low-T routing change
    ├── 2026-07-08-post-result-cross-sell-complement-rule.md ← Kit 1 post-result cross-sell = Kit 2 (complement rule)
    ├── 2026-07-17-retest-cadence-table.md     ← PROPOSED retest cadence (three buckets), pending Ewa sign-off
...
| T 12–<15 nmol/L borderline (Kit 1 or Kit 3) | Supplement waitlist (`interested_in_product = daily-stack`) + borderline nurture opt-in (seq-03d) | Daily Stack | **Kit 2 cross-sell** (Kit 1 buyer; complement rule 2026-07-08: was "Kit 3 upsell") | Borderline framing: "worth monitoring". Card stays `normal-testosterone`; not a clinical reclassification. T-monitoring is handled by retest + seq-03d, so the cross-sell adds the complementary energy panel. |
| T normal, all markers in range | Supplement waitlist (`interested_in_product = daily-stack`) | Daily Stack | Retest in 6-12 months | No medicinal-claim framing. All-clear (Bucket C) cadence per `results-engine/2026-07-17-retest-cadence-table.md` (PROPOSED, pending Ewa sign-off). |
| Low Vit D or Low B12 (Kit 2 or Kit 3) | Supplement waitlist (`source_marker = low-vitamin-d` or `low-b12`, `interested_in_product = daily-stack`) | Daily Stack | None | EFSA claims only. Honest OTC suggestion shown alongside the waitlist CTA in Phase 0a. |
...
| Low Ferritin (Kit 2 or Kit 3) | GP referral prompt | GP referral prompt (unchanged) | None | Ferritin deficiency requires clinical investigation |
...
| Quiz complete, no purchase | Recommended kit CTA | Recommended kit CTA (unchanged) | None | Kit recommendation from `customer.quiz_recommended_kit` |
...
> **Superseded 2026-07-08: Kit 3 is NOT a post-result cross-sell.** The "Kit 3 is the Kit 1 post-result upsell" framing is retired: a man who just tested testosterone is not sold a kit that re-tests it. The Kit 1 **post-result** cross-sell is **Kit 2** (the complementary energy panel), unconditional on a normal-T Kit 1 result. Kit 3 remains a **front-of-funnel default** (the test-selector's broaden-up recommendation for people who have not tested yet) and a **direct-traffic** product via `/lp/hormone-recovery`; it is never offered as a post-result cross-sell. Canonical: `results-engine/2026-07-08-post-result-cross-sell-complement-rule.md`.
...
- **Biomarker → EFSA-supplement loops** (Phase 0 self-sustaining, no CQC dependency): lead loop = **Omega-3 Index → Omega-3 (EPA/DHA)** (finger-prick + retestable, EFSA heart/brain/BP/triglyceride claims, strong demand). Analysis: `supplements/biomarker-supplement-loops.md`; dedicated DRAFT spec: `supplements/omega-3-loop-spec.md`.
...
- `/decision-sweep`: propagate a pricing, threshold, biomarker-rule, or results-engine change through every affected doc.
- `pricing-strategy`: kit and supplement pricing/packaging logic.
- `customer-research`: ICP and buyer-need synthesis behind kit/supplement alignment.
- `dataviz`: charts for demand analysis and results-engine reporting.

**MCPs & tools:**

- **Stripe** (MCP, connector): product/price/SKU definitions and reconciliation for kits and supplement subscriptions.
- **supabase** (MCP, wired, read-only): results-engine data, biomarker/results tables, order data (project phqrjtnflovicgkngieu).
- **dataforseo** (MCP, wired): supplement/kit demand analysis (e.g. `supplements/supplement-demand-analysis.md`).
