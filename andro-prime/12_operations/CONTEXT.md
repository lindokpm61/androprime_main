...
- **`automation/shot.js`** (local script, no MCP): the way to actually SEE a rendered page. **No browser MCP is wired here and `/plugin` is unavailable in this environment**, so nothing is installable through the plugin system; this script drives system Chrome through puppeteer-core instead. `node andro-prime/12_operations/automation/shot.js <file-or-url> --selector "css" --nth all --theme both`. Shoots per-element and per-theme, waits on webfonts, disables subpixel text fringing, and warns on horizontal body overflow. **A local path may carry a query string** (fixed 2026-08-28): the design mockups read `?still=1` and `?t=light|dark` off their own URL, and the existence check used to reject the whole argument as "no such file", which made the tool unusable on exactly the pages it exists to photograph. **Use it rather than `chrome --headless --window-size` for any narrow viewport**: Windows clamps the window to about 518px inner width and then crops the PNG to the width you asked for, so a 390 capture is a 518 render cropped to 390 and every overflow it appears to show is an artefact. **Reaching a page behind auth takes two things and neither is optional.** `--cookie name=value` carries a session and `--resolve "MAP app.andro-prime.com 127.0.0.1:<port>"` makes the request arrive on the host the middleware routes on; without the second, `/auth/*` and every `(app)` route 308 to production. Minting the cookie is deliberately NOT `shot.js`'s job — a scratchpad helper drives the real login form and prints the `sb-*` cookies as ready-made `--cookie` arguments, and the consent banner is `fixed` over the submit button so it needs `ap_cookie_consent` seeded into localStorage first. **Assert TWO strings on any gated capture**: one that exists only in the authenticated shell (`Log Out`) and one that identifies the page. A fresh headless browser with no session photographs the login page and that looks like a successful capture — and the login page's title is *"Access Your Results"*, so the dashboard's own `metadata.title` passes on it. 🔴 **The walk used to capture mid-scroll, and it silently invalidated every tall-page screenshot taken before 2026-09-17.** The walk ended `window.scrollTo(0, 0)` then paused 200ms, while the site sets `scroll-behavior: smooth` on `<html>` and does not relax it under `prefers-reduced-motion` — which this tool emulates by default — so the scroll animated and the PNG was taken wherever it had reached: **`/` 4676px down, `/faq` 6538px, `/kits` 2768px**, each settling to 0 a second later. Exit 0, no warning, file written. The walk now forces `scroll-behavior: auto` for its duration, restores it, and **prints the rest position if the page did not return to the top** — if you ever see that warning, the capture is not of what you asked for.
