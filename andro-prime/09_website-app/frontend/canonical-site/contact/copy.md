# Contact — Copy Document

**Page URL:** `/contact/`
**Target Audience:** All ICPs. Post-purchase support, pre-purchase questions, general enquiries.
**Tone:** Keith's voice. Warm, direct, human. Not a corporate contact form with a ticket reference number.
**Goal:** Remove friction for anyone who has a question. Reassure that a real person will respond.
**Status:** Draft — contact details to be confirmed before publishing.

---

## 1. Hero Section

**Eyebrow/Label:** GET IN TOUCH

**H1:** Got a question? We'll answer it.

**Subheadline:** This isn't a bot. There's a real person on the other end. Email us and you'll hear back within two working days.

---

## 2. Contact Options

### Primary contact

**Email:** hello@andro-prime.com
**Response time:** Within 2 working days. Usually faster.

**[CTA Button or mailto link]:** Send us an email

---

### What to contact us about

Use these to route your question to the right place before you write to us.

**Kit orders and delivery**
Questions about your order status, delivery, or kit contents. Include your order number if you have it.

**Results and your dashboard**
If your results haven't appeared, something looks wrong, or you want to understand what a result means.

**Supplements and subscriptions**
Questions about your subscription, billing, cancellations, or pausing deliveries.

**Founding member programme**
Questions about the founding-member list or the clinical launch timeline.

**Privacy and data**
Requests to access, correct, or delete your data. See also our [Privacy Policy](/privacy/).

**Everything else**
Media, partnerships, or anything else — hello@andro-prime.com covers it all.

---

## 3. What Happens When You Email Us

We read every email. You won't get an auto-reply that closes your ticket and tells you to check the FAQs.

If you're asking about your results, we may ask Dr Ewa to weigh in. She's involved in the business, not a name on a website, so if your question needs a clinical eye, it gets one.

**[PLACEHOLDER: Consider adding a contact form here as an alternative to email. Reduces friction for people on mobile. Fields: Name, Email, Subject (dropdown: Order / Results / Subscription / Founding Member / Data / Other), Message. Conditional routing on submission by subject.]**

---

## 4. Frequently Asked

These are the questions we get most often. Worth checking before you write to us.

**Q: My results haven't appeared in my dashboard. What do I do?**
A: Results typically appear within 48 hours of the lab receiving your sample. If it's been longer than that, email us with your order number and we'll look into it.

**Q: I want to cancel my subscription. How do I do that?**
A: Log in to your account, go to Subscriptions, and cancel from there. Cancel before your next billing date and you won't be charged for the following month. If you're having trouble, email us and we'll sort it.

**Q: Can I speak to a doctor about my results?**
A: Dr Ewa Lindo reviews our clinical protocols and results report copy. At this stage, we're not offering one-to-one clinical consultations. If your results raise something that needs medical attention, we'll tell you that clearly in your dashboard, and we'd encourage you to speak to your GP.

**Q: I have a complaint.**
A: We'd rather hear it than not. Email hello@andro-prime.com and we'll deal with it properly. If you're not satisfied with how we handle it, you can escalate to an approved ADR scheme or the relevant regulatory authority.

---

## 5. Data and Privacy

For data-related requests including access requests, deletion, and corrections:

**Email:** privacy@andro-prime.com
**Response time:** Within one calendar month, as required by UK GDPR.

Full details of your rights are in our [Privacy Policy](/privacy/).

---

## 6. Business and Press

Working on a story about men's health, hormones, or the state of GP testing? We're happy to talk.

**Email:** hello@andro-prime.com with "Press" in the subject line.

---

## Page Footer (above site footer)

**No chatbot. No ticket system. No hold music.**
Just an inbox and two working days.

---

## Compliance Notes

- Do not imply clinical consultations are available. Use the exact framing in FAQ Q4: Dr Ewa reviews protocols and results copy, not individual consultations.
- Do not reference TRT as currently available.
- Founding-member language must describe an opt-in mailing list, not a paid deposit. The £75 deposit was retired on 2026-05-08.

---

## Placeholders Outstanding

| Section | What's needed | Who provides it |
|---------|--------------|-----------------|
| Contact form | Decision: form vs email link only | Keith |
| hello@andro-prime.com | Confirm inbox is active before launch | Keith |
| privacy@andro-prime.com | Confirm separate inbox or alias | Keith |
| Press contact | Confirm if hello@ handles press or separate address | Keith |
