...
- **Suppression (TO ADD before activation):** exclude anyone who has fired `subscription_started` so a supplement subscriber does not get both this and the Day-90 seq-04 e5 prompt. Not yet wired on the draft — add as a trigger person-filter or global exit condition at activation. Low-risk to omit (an all-clear person rarely subscribes), but honour the spec.
