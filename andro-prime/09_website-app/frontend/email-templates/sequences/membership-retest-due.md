...
**Status:** 🔴 **DRAFT. NOT APPROVED, NOT ACTIVE.** Copy drafted 2026-09-13, compliance pre-flight run the same day. Customer.io campaign built as a **DRAFT**. Activation is gated on: Keith's business sign-off, the `membership_retest_due_at` attribute being stamped (not built yet, see below), `MEMBERSHIP_ENABLED`, **`ACCOUNT_ADDRESS_ENABLED`** (see below), **`P1` — the solicitor's answer on the DMCC Act 2024 Part 4 subscription regime, because Keith's 2026-09-13 ruling keeps the entitlement paragraph and ship-gates it rather than cutting it (outstanding item 3)** — an attribute-live test send, and a human go/no-go. **P1 is the only gate on this list that is not ours to move.**
...
| 3 | ✅ **SETTLED 2026-09-13 — the entitlement paragraph stands, and it ships when P1 does.** Keith's ruling: keep it, ship-gate it, do not cut or reword it. It remains contract copy where no contract exists (`terms-and-conditions.md` has no membership section), so it takes the CA-026 D2 treatment exactly. **The consequence is that P1 is now an activation gate on this campaign** — added to the gate list above — and it is the only one of the four that is not ours to move | Settled, Keith | P1, now a gate |
...
No CA number is claimed; ~~the highest in use is CA-046 and nothing is reserved~~ **(CORRECTED 2026-09-15: CA-047 is now open for the retest cadence table; nothing else is reserved)**. ~~**No new Ewa ask should be raised**: one is drafted and unsent, and items 1 and 2 belong in it.~~ ⚠ **STALE 2026-09-15: that drafted ask was SENT and ANSWERED** (`1: A 2: A 3: A 4: C 5: A`, CA-047). **Items 1 and 2 did not ride it and are still owed**, so a new ask is now the only route for them. Check them against CA-047 before raising: her Q3 = A (GP-routed results get no Andro Prime interval, the card shows the referral) and Q4 = C (cadence is per marker, never per panel) may already answer part of what they ask.
