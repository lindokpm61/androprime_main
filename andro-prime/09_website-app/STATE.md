...
`blog-skin.css`, so F lost that frame and `blog-F.html` stands as the record. Counting them as
...
- **Copy defects**, all live: the panel understated by FAI and Albumin on `/how-it-works` and `/faq`
  (and on `/faq` it contradicts the CA-026 block on the same page); the "three questions, under a
  minute" selector description on `/kits`, in two places.
...
### 🔴 THE PANEL IS UNDERSTATED ON BOTH LEARN PAGES, BY THE SAME TWO MARKERS
...
description on `/kits`. Approving the frames that display those sentences is not approving the
sentences, and each is still owed as a copy fix.

**`blog-F.html` is NOT on that list and today does not revive it.** Keith put it down on 2026-08-27
...
## ▶️ PICK UP HERE: the journey mockup, frame by frame (handoff rewritten 2026-08-28)
...
`blog-F.html` is the record of a frame F lost and is NOT adopted (the blog keeps `blog-skin.css`,
Keith 2026-08-27). The blog index and article are OFF the board, not outstanding.

🔴 **THE GATE KEITH SET ON 2026-08-26 IS SATISFIED.** His sequence was: *the mockup must first
...
1. 🔴 **The panel is understated on two live pages, by the same two markers.**
   `/how-it-works` lists Kit 1 as three markers (Total T, SHBG, Free T) where every commerce surface
   says five; `/faq`'s marker table lists seven rows and its copy commits to "the seven" where the
   panel is nine. **FAI and Albumin are missing from both.** On `/faq` this contradicts the CA-026
   approved block on the same page, which says the price buys "the markers that matter for men,
   including free testosterone via FAI". Substance in the Learn entry in this file. **Fix this one
   first**: it is live, it understates the product, and one half of the contradiction is signed copy.
