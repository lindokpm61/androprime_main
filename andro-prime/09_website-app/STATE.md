...
⚠ **TWO DEVIATIONS ARE NOW PROPS RATHER THAN MISSING CLASSES, AND ONE IS A
QUESTION FOR KEITH.** The kit pages' `heroPad={62} padBottom={44}` is deliberate
and documented. But `/kits` and `ArticleLayout` close **without the `.f-rise`
reveal** that `/how-it-works`, `/membership` and `/blog` have on the same element.
That was invisible while it was an absent class; it is now `reveal={false}` in two
places. **Nobody has ruled whether the two closes should animate.** Left as found
rather than normalised, because adding motion to two closes is a design change and
not a refactor.
