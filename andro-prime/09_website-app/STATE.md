...
1. **The pre-flight scanner rule for the retired status vocabulary.** ~20 minutes and it is the open
   half of the correctness work. The words were removed from `/`, `/kits/testosterone` and
   `F-field.html` this session, but `compliance-preflight/scan.js` still has no rule for them, so the
   defect can return exactly the way it arrived, by a port from a mockup, with nothing mechanical to
   catch it. Source the word list from `lib/results/resultSeverity.ts`'s label map so the scanner and
