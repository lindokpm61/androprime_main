...
## ▶️ PICK UP HERE: the first F slice is built and running (handoff rewritten 2026-08-30, evening)
...
### 🔴 `/how-it-works` HAS NOT HAD THAT COMPARISON RUN

It was built by the same method as the first `/kits` pass, from `learn-F.html` Frame S, **before** the
drift was found. It very likely carries the same class of error. **Diff it against its frame before
building anything new.** That is the cheapest task on this list.
...
1. 🔴 **Diff `/how-it-works` against `learn-F.html` Frame S** with the two-shot method above. Built
   pre-lesson, most likely place for the same drift.
...
3. **The mockup-vs-primitives reconciler**, plus Keith's ruling on app-vs-marketing density.
...
5. **The headline faces.** Newsreader is a stand-in. Austin (Commercial Type) over a humanist sans
