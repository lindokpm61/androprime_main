...
### The next move (handoff rewritten 2026-08-29, end of session)
...
1. 🔴 **The FAI framing is unswept on three commerce pages.** `/kits/testosterone:388`,
   `/kits/hormone-recovery:338` and `/lp/hormone-recovery:126` still carry the framing the 2026-07-30
   ruling corrected in the spec but never swept into the pages, so they now disagree with `/faq` and
   with the results engine. A `/decision-sweep` of an approved decision, claim reduction, no fresh
   sign-off needed. Left for Keith because it touches a paid-ad LP.
