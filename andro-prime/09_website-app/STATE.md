...
## ▶️ PICK UP HERE: the rebuild has started (handoff rewritten 2026-08-30)
...
### The next move (handoff rewritten 2026-08-30, end of session)

**Read this block and nothing else to start.**

🔴 **NOTHING IS DEPLOYED, AND THE BRANCH SITUATION NEEDS SORTING BEFORE ANY PUSH.** Three commits sit
on `main` unpushed (`089f93f`, `2bea2ea`, `4a43864`). One rebuild commit sits on
`redesign/direction-f` (`ea662ce`). **A push to `main` is a deploy** (`CLAUDE.md`, Coolify), so none of
it has shipped. 🔴 **A parallel Claude session committed two of its own commits onto
`redesign/direction-f` by accident** (`9d70789` demo-account prototype, `538a289` the brand-lead A2
sweep) because both sessions shared one working tree and one checked-out branch. **Those two are
docs-only, unrelated to the redesign, and belong on `main`.** They were left alone deliberately: the
other session was still writing, and rewriting shared history under a live session is worse than the
mess it fixes. Sort this before pushing anything.
...
4. ✅ **`/kits/testosterone` rebuilt in Direction F** (`ea662ce`), the first page. Frame P.
   Primitives extracted to `frontend/styles/components/f-primitives.css`, which is the reusable layer
   the other 45 pages inherit.

**What the next chat should pick up, in order of cost to leave**

1. 🔴 **THE TYPEFACE IS THE BLOCKER, AND IT IS KEITH'S.** Direction F is drawn in **Geist**; the site
   ships Inter, Merriweather and JetBrains Mono. F is also a **sans-body** system, so Merriweather has
   no role in it, and F's headings are **weight 700 sentence case**, not Inter Black uppercase. This
   was never ruled: `STATE.md` records the switch happened *"because `high-end-visual-design` bans
   Inter outright"* (a lint rule in the prototyping tool) and
   `design/homepage-direction-brief.md` §8 flags the finding as **"Keith's to reopen if he wants"**.
   **Inter is load-bearing elsewhere: the logo glyphs are Inter Black outlined to paths**, so this
   decision is tied to the logo one below. Recorded as open in `styles/tokens/typography.css`.
   **Because every F rule references `--font-sans` / `--font-mono` rather than a family name, ruling
   Geist is a three-line change plus `app/layout.tsx`, and nothing else.**
2. 🔴 **The logo mark's radius, still open** (from 2026-08-29). F rounds the AP square 9px / 7px.
   `visual-identity.md` lists **"round the square's corners"** among the things not to do to the mark,
   and the mark is on the packaging sleeves rendered this week, the icon set and the OG cards.
   **Three alternative marks were generated for review on 2026-08-29** and are in
   `02_brand/assets/logos/explorations-2026-08-29/` with a README: A-prime notational, an aperture
   dial, and an editorial Didone A. **None is an asset** (raster, not vector) and **none has been
   tested at 16px**, which is the gate the June exploration used. Record:
   `02_brand/2026-08-29-direction-f-supersedes-v2-non-negotiables.md` §5.
