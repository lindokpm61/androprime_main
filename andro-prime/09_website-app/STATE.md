...
**Draw `/kits` and the three kit pages.** That is the rest of the CHOOSE stage and the biggest single
block left on the board: `design/journey-inventory.md` had them as "one template, three kits" and
there is no template and no `[kit]` route, just three hand-written pages of 530, 882 and 463 lines
plus a 615-line index. **Run the lifecycle check first** (first ten lines of the entry file, grep
`docs/` for the route name, grep for inbound links): it has caught a dead route twice in two
sessions, `/activate` and `/founding-member-status`. Then enumerate before drawing, and screenshot
the live page beside the redraw. After Choose, what is left is **Learn** (3 frames), **Buy** (2) and
the **five auth frames**, where `/auth/consent` carries approved CA-018 health-data copy.
