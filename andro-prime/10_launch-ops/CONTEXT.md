...
(`checklists/` does not exist; `dashboards/` and `weekly-reviews/` exist but are empty and unused (weekly reviews and dashboards are not maintained here; task/KPI status lives in ClickUp + STATE.md).)
```
...
Review every Monday. KPI reviews run in ClickUp, not a markdown folder (`weekly-reviews/` exists but is empty and unused).
...
1. Pull data from **GA4** (traffic, CTR), Stripe (kit sales, subscription MRR, CAC), Supabase (result counts, supplement conversion rate).
...
4. Record the KPI review in ClickUp, not a markdown folder (`weekly-reviews/` exists but is empty and unused).
5. Update the financial model at `../01_strategy/financial-model/option-4-financial-model-2026-05-08.md` if revenue or cost figures have changed. Version the model doc's header/status field in place, do not create a new file.
...
1. Add it to the relevant gate checklist in `implementation-checklists/`.
...
**Financial model:** Always update `../01_strategy/financial-model/option-4-financial-model-2026-05-08.md` in place, do not create a new file. Version the model doc's header/status field with the date of each edit.
...
- `/wrap`: end-of-session close-out (reconcile STATE/CONTEXT, update ClickUp, commit and push by path).
- `/task-observer`: capture skill-definition friction during a session.
- `analytics-tracking`, `ab-test-setup`: KPI/measurement setup and experiment design.
- `dataviz`: dashboard and KPI-report charts.
- `schedule`, `loop`: recurring gate/KPI checks and scheduled cloud agents.

**MCPs & tools:**

- **clickup** (MCP, wired): CANONICAL task tracking. Live sprint list `901217968514` in workspace `90121729875`; every ClickUp call must pass that workspace id. Do not keep a parallel open-task list in markdown.
- **Stripe** (MCP, connector): revenue KPIs and Gate 0A/0B invoice-count checks.
- **supabase** (MCP, wired, read-only): data-side KPIs and readiness checks.
- **GA4** (`G-D5M4J5M3F6`, live): traffic/analytics KPIs. Not an MCP; the live stack is GA4 + Sentry (Plausible/Meta Pixel/Clarity are NOT wired).
