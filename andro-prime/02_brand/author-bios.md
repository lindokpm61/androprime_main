...
**Scope of that sign-off:** originally three surfaces carrying the same personal-story framing, cleared together and as-is. This bio on `/authors/keith-antony`, the About section of Keith's LinkedIn profile, and the LinkedIn post `instrumentation-problem` (`7487903563306733568`). **Amendment 1 (Keith, 2026-08-16) adds a fourth: Keith's X account `KeithAndroPrime`.** X was not a configured channel in July, so its absence recorded the sequence of events rather than a decision about the surface; three X batches paused on that ambiguity before it was settled. The same amendment relaxes "verbatim" to cover tense and sentence-split variants that add no proposition, because the X posts open with a re-tensed, re-split form of the About section's first line. ⚠️ **The LinkedIn HEADLINE is still not a named surface** and carries near-verbatim approved copy; that gap is open and recorded in the approval record. The symptoms-to-testosterone framing was put to her explicitly against the Kit 1 scoping rule and cleared without amendment. The closing line "I got the right support. It changed everything." stays here as approved; it is deliberately absent from the LinkedIn version, and that divergence is approved rather than an error to reconcile.
