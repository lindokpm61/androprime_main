...
├── references/narrative-devices.md ← TWO layers. (1) Nine structural devices from the spoken corpus (ordinary-build-then-rupture, physical object for an invisible state, flat close, etc.), used while drafting a passage; extends tone-of-voice.md §2. (2) "The structural audit" (v1.1, 2026-08-10): six whole-piece audits, the intervention menu, and the rotation rule, run at voice-pass time by /article step 5c and /script step 4b. STRUCTURE ONLY: contains no biographical content and is not a licence to reproduce any.
...
| No AI tells | Seven hard fails on top of the voice checklist, each a rewrite not a score: throat-clearing openers ("Here's the thing"), meta-joiners ("The rest of this article covers"), inanimate subjects doing human verbs ("the marker tells a story"), narrator-from-a-distance ("Nobody designed it this way"), vague declaratives ("the implications are significant"), negative listing ("it's not about energy, or sleep, or libido"), and empty inline-header lists (a bolded header whose sentence only restates it). Full list `tone-of-voice.md` §9a. Section 9 asks *does this sound like Keith*; §9a asks *does this sound like a machine*. Copy can pass one and fail the other. |
| Shape, not just words | §9 and §9a both test words, and a draft can clear both and still read as machine-made, because the durable AI signal is the **shape** of a piece and its similarity to recent pieces. `tone-of-voice.md` §9b points at the structural audit in `references/narrative-devices.md`: six audits, an intervention menu to rotate, and the rotation rule. Two consequences bind everywhere: the "It's not X. It's Y." reframe is **once per piece, not once per section**, and the section arc (hook, question, reframe, close) is a **default shape available to a section, never a template every section takes**. Run by `/article` step 5c and `/script` step 4b. |
| Voice carve-outs | Generic anti-AI writing tools and skills ban four devices this brand **mandates**: the rule of three, the "It's not X. It's Y." reframe, deliberate sentence fragments, and question-led openers. When an outside tool, skill, or reviewer flags one, the house rule wins. Do not install a third-party prose skill that fights them, and do not "fix" them out of a draft. Table in `tone-of-voice.md` §9a. |
...
| Logo / mark | 🟢 **DIRECTION CHANGED 2026-08-30: Keith approved the "Interlocked AP"** (A and P as one glyph sharing a stem, **no container**), `assets/logos/interlocked-ap/`. **Direction only: no vector masters exist, so everything below is still what ships.** Previous, and still live: **Refined Monogram** (chosen 2026-06-12): black square + white "AP" (Inter Black) + "ANDROPRIME" wordmark at `letter-spacing: 0`. Masters: `assets/logos/refined-monogram/`. Default = **solid** mark; the **outlined** variant (`icon-outline*.svg`) is a sanctioned exception to the "no effects" rule for **large format only (≥~25mm)**: favicon/small/UI keep the solid mark. Shipped as `components/shared/Logo.tsx` in `09_website-app`. See `STATE.md`. |
| Blog exception (`.blog-skin`) 🔴 **ENDING: superseded 2026-08-29** when Keith approved `blog-F.html` and the blog adopted Direction F. Still live, not yet deleted; the exception retires with the rebuild. | The blog runs a scoped **brutalist editorial** category (cream `#f4f4f0` surface, hard offset block-shadows, mono "SYS:" labels): a documented departure from the white-only / no-shadow rules, namespaced under `.blog-skin` only. **Accent red is DROPPED** (red is reserved for the results-dashboard critical/GP-block status; decorative red near health copy collides with that meaning + ASA risk). GP-referral boxes stay strictly black-on-white. Implementation: `09_website-app` `styles/base/blog-skin.css`. |
...
**`messaging-framework.md`:** Contains ICP-level positioning and value prop statements. When writing copy for a specific ICP, check this file first. Do not duplicate its content into ad briefs; reference it. The lead pillar (v1.0, 2026-05-19) is the **data-ownership inversion**: "your results, in your dashboard, tracked over time, and yours to keep; you see everything we see"; blood-data-first is kept as the method beneath. Pillars: 1 Patient-owned data (**proof layer since 2026-08-30, not the lead**), 2 Comprehensive men's health, 3 Clinical credibility, 4 Premium. **The category noun is "a UK men's health company"** (Keith, 2026-08-30): not "wellness", which is the Phase 0 regulatory tier rather than an identity, and not "testing company", which forecloses the record and the membership.

> ⚠️ **LEAD SUPERSEDED 2026-07-22: conflict-free is now the governing position.** Keith adopted the **conflict-free positioning** (`../01_strategy/2026-07-22-conflict-free-positioning-decision.md`), approved wording **CA-026** (`2026-07-22-conflict-free-wording-pack.md`). Customer copy now leads speed / no-GP first, money honesty second (one price; any result that needs a doctor goes to a GP and earns us nothing; no result changes what we offer or what it costs). The **Patient-owned data** pillar is retained as a supporting pillar inside conflict-free, no longer the lead. **AMENDED 2026-08-30 (Keith, ruling A2, `../01_strategy/2026-08-30-brand-lead-after-the-monitoring-thesis.md`): it is a PROOF LAYER, not merely a privacy receipt** — the demonstration of conflict-free, permitted in customer copy as demonstration and still not permitted to open it. The sharp line "no reason to sell you testosterone" is press/GEO only; on-site press form is E2. See the `messaging-framework.md` banner for the full rule set.
