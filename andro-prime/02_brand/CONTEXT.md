...
├── references/narrative-devices.md ← Nine structural devices from the spoken corpus (ordinary-build-then-rupture, physical object for an invisible state, flat close, etc.). Extends tone-of-voice.md §2; read by /article at draft time. STRUCTURE ONLY: contains no biographical content and is not a licence to reproduce any.
...
| No AI tells | Six hard fails on top of the voice checklist, each a rewrite not a score: throat-clearing openers ("Here's the thing"), meta-joiners ("The rest of this article covers"), inanimate subjects doing human verbs ("the marker tells a story"), narrator-from-a-distance ("Nobody designed it this way"), vague declaratives ("the implications are significant"), negative listing ("it's not about energy, or sleep, or libido"). Full list `tone-of-voice.md` §9a. Section 9 asks *does this sound like Keith*; §9a asks *does this sound like a machine*. Copy can pass one and fail the other. |
