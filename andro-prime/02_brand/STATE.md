...
> **2026-09-13 — RULING B'S DOC SWEEP RAN. The docs were mostly already right; the LIVE SITE is what
> still says "wellness".** Keith asked for the men's-wellness-to-men's-health sweep across the
> documentation.
>
> - **Docs: one genuine change.** 1,430 occurrences of "wellness" repo-wide, 83 positioning-shaped,
>   and most of those are **contrastive** ("not a wellness brand"), which ruling B makes *correct*.
>   Only `../01_strategy/phase0-v7-implications.md` actually asserted the superseded noun. Updated.
> - 🔴 **The code fix never reached production.** `main` still serves *"Andro Prime is a wellness
>   information service"* (three times on the homepage). The 2026-09-02 fix is on
>   `redesign/direction-f` only, and `PRODUCT.md` recorded it as "✅ Implemented" without naming the
>   branch. Corrected there.
> - 🔴 **ESCALATED to Keith:** `canonical-site/terms/index.html` and `canonical-site/privacy/index.html`
>   carry the superseded sentence on **both** branches, were on nobody's list, and are published legal
>   documents pinned by `verify-legal-text.js`.
> - ⚠ **The two senses of "wellness" stay separate.** This file's own 2026-08-30 entry already says it:
>   *"wellness is the Phase 0 regulatory tier."* The regulatory tier, the wellness-versus-clinical
>   entity split in the financial models, and the not-a-diagnosis disclaimers are a different term and
>   were deliberately untouched. A blind replace would have inverted the brand's own positioning lines
>   and eroded the Phase 0 boundary in one pass.
> - **Question A2's sweep items remain unrun.** Only question B was swept.
...
_Last updated: 2026-09-09 (🟢 **THE KIT 2 ORIENTATION SERIES EXISTS, AND IT FOUND THE ANGLE AT WHICH GENERATED PACKSHOTS STOP BEING TRUSTWORTHY.** Keith asked for a product shot list of the Kit 2 pack from different positions: flat, angled, side. Eight camera positions built on the canonical render (job `10f799cd`); **five are verified and usable** — hero 3/4 left (S1), front elevation (S2), overhead flat (S3), 3/4 right (S4), low hero (S8) — plus a true 90° profile (S6) that passes but is a blank bone rectangle. Full list, job_ids and per-shot verdicts: `2026-09-09-kit2-product-shot-list.md`; files at `assets/packaging/renders/kit2-shotlist-2026-09-09/`. 🔴 **THE FINDING THAT MATTERS: NEITHER `gpt_image_2` NOR `gpt_image_2_5` CAN HOLD A WIDE TWO-COLUMN TABLE'S ROW PAIRING UNDER STRONG FORESHORTENING.** Every character renders correctly and correctly spelled while the **unit column slides vertically against the name column** — a spell-check passes it, only a structural read catches it. Near-frontal is reliable (0°, overhead-square, ~25°, shallow low-angle all passed **first time**); it breaks past **roughly 40°** of panel foreshortening. Three attempts, two models, same direction every time — at 40° the whole unit column sat one row high and FERRITIN lost its unit; at 75° the units crossed the rules their names sat below — so by the runbook's own rule this is **capability, not prompt, and re-rolls should stop**. ✅ **The route for steep angles is already in this file**: the three.js render is the geometric ground truth, and a real perspective transform projects both columns through one matrix by construction, so it cannot make this mistake. Geometry first, generative pass second for material and light — the same pipeline this STATE already settled on for the supplement label, applied to a different failure. That is a build job, not a prompt job. ⚠️ **Second finding: the carton is printed on the FRONT PANEL ONLY** — top, back and both sides are plain unprinted bone. That is why S6 has nothing to show. No shelf presence in any orientation but front-on, and a stack reads as anonymous card; **if retail or a stacked hero matters the fix is artwork, not photography.** Keith's call. 🔴 **KITS 1 AND 3 ARE STILL ON THE SUPERSEDED MARK** — verified by eye on `kit-1-testosterone-3d.jpg`: boxed `AP`, one-word `ANDROPRIME`, and the Dr Ewa line, all three. **Any three-kit family line-up is blocked** until both are re-run through the correction that fixed Kit 2 (~22 credits). 🔵 Catalogue re-verified this session: **`gpt_image_2_5` now exists and is CHEAPER than `gpt_image_2`** (7.5 vs 11 credits at 4k/high) and carries `auto` in `aspect_ratios`, which `gpt_image_2` does not — the better default for near-frontal work. Spend this session: 106.5 credits. Earlier today: 🟢 **THE KIT 2 PACK ART NOW CARRIES THE REAL IDENTITY, AND THE OLD ONE WAS BREACHING THE MARK'S OWN SPEC.** Keith asked for the Kit 2 pack shot to be rebuilt on the real logo. The previous render (generated 2026-08-20) put "AP" inside an **outlined square** and set the spine as one word, "ANDROPRIME". The interlocked-AP README is explicit that the mark takes "no enclosing square and no container of any kind", so the boxed version was a spec breach, and the wordmark is **"ANDRO PRIME", two words**. Both fixed, plus the line "Thresholds signed off by Dr Ewa Lindo, GMC-registered GP" removed at Keith's instruction. Rebuilt on `gpt_image_2` high/4k, 3312x2480, job `10f799cd-32ab-4284-887f-9d41dd30c08c`; every other line preserved and verified correctly spelled at zoom. Render at `assets/packaging/renders/kit2-pack-interlocked-ap-2026-09-09.png` with a mark-vs-vector comparison beside it. ⚠ **These renders are gitignored from 2026-09-09** (heavy, superseded often); the file is on disk and recorded here with its job_id rather than committed as bytes. ⚠ **Do NOT build pack art from `SOURCE-*.png`.** Keith pointed at those files and the README's own red-flag section says they are generative-model output, "not shippable and must not be placed on any surface", and `SOURCE-mark-only.png` is additionally **clipped, 60px off the foot of the P stem**, which is the one landmark the mark is about. The drawn vectors (`icon.svg`, `lockup-light.svg`) were used instead, rasterised via the committed `shot.js`. ⚠ **The generated monogram is CLOSE but is not the vector**: diagonal weight, crossbar height, dropped stem and bowl all match, but it is marginally narrower with a slightly smaller bowl. Fine for a mockup; **for any definitive surface composite `icon.svg` rather than generating it.** 🔴 **Two claims ON the pack need verifying before it appears in any ad**, and neither is a design question: the boxed **IVD plus CE · UKCA** marks are a legal conformity declaration, and **"ANALYSED IN A UKAS-ACCREDITED UK LABORATORY"** is a factual claim about the lab. Earlier, 2026-09-02: 🟢 **THE LOGO IS DRAWN, DECIDED AND INSTALLED.** Keith ruled the lockup
...
- 🟢 **v8 IS BUILT TO THE REAL TEMPLATES AND IS THE FIRST SLEEVE ARTWORK THAT IS ACTUALLY PRINT-READY (2026-09-09).** One generator, `assets/packaging/gen-sleeve-v8.js`, emits **8 files**: two cartons x two palettes x {proof, print}. **Capillary 169 x 304** (panels 15/122/23/122/22, page 179 x 314 with 5 mm bleed) and **Tasso 178 x 400** (panels 7/144/53/144/52, page 188 x 410), both verified in the written PDFs at 178.90 x 313.94 and 188.04 x 409.96 mm. Each flat now has a **real 23 mm spine panel**, black, carrying the wordmark horizontally, with the **back rotated 180°** so it reads correctly once wrapped. The die aperture is drawn at **118 x 67 r5 on the BACK only**, with the 5 mm artwork-bleed limit shown; all aperture guides are **proof-only**, so the print files carry artwork alone. Marks are the **drawn vectors** (`icon.svg`, and the traced wordmark lifted from `lockup-light.svg`), not type. ⚠️ **The `proof` files carry crease/trim/die guides and are for us; the `print` files are what goes to Mega-Pak.** 🔴 **AND THE FONT TRAP THAT v7b SHIPPED IS NOW CLOSED.** v7b declared Inter / JetBrains Mono / Merriweather and carried **no `@font-face` at all**, so Chrome substituted silently and those PDFs embed **Arial-Black, Arial-BoldMT, ArialMT, Consolas, Consolas-Bold and TimesNewRomanPS-BoldMT** — not one intended face, and invisible in every visual review. Adding `@font-face` was not enough either: **Chrome refuses font requests from a `file://` origin**, so it still fell back. `12_operations/automation/proof-pdf.js` now serves the folder over `127.0.0.1`, **asserts every declared `@font-face` actually loaded before writing the PDF**, and takes page size from the document's own `@page`. v8's print PDFs verifiably embed **Inter, JetBrainsMonoRoman, JetBrainsMonoRoman-Bold and Merriweather-Bold-18pt**, and nothing else. Faces are OFL, fetched by `assets/packaging/fetch-fonts.sh` and gitignored. 🟡 **Still open before it goes to a printer:** the **aperture's horizontal position** (drawn at 24.09 mm from the back's left trim, derived from our `.eps` reading rather than measured off the template) needs confirming with Vitall; **CMYK conversion and text-to-outlines** are Mega-Pak's asks and have not been done; and **which carton is actually ours** — the evidence says Capillary, see below.
- 🔴 **BIGGER THAN THE APERTURE: THE ASSUMED FOLD LINE IS WRONG, AND IT HAS PROPAGATED INTO EVERY PACK IMAGE WE HAVE (2026-09-09).** The Route B proof records *"Fold: **152.00 mm** from trim top — **assumed**, no crease layer exists in Vitall's file"*, and it derives **folded face 179.00 x 152.00 mm** from halving the 304 mm flat. **The template says the panels are 122 / 23 / 122 / 22.** There is a **23 mm spine**, so the faces are **122 mm, not 152 mm** — the proof's front face is **30 mm too tall**. The proof flagged this as an assumption and was right to; the template disproves it. **Consequences, in order of blast radius.** (a) **The Route B fronts and back must be RE-LAID-OUT, not nudged** — 30 mm out of 152 is 20% of the face height. (b) **Every generative pack render inherits it**: the approved Kit 2 render measures a **1.196 : 1** front face against the proof's 179/152 = 1.178, and the truth is **169/122 = 1.385**, so the marketing pack art is **~14% too tall** and needs to be **~18% wider relative to its height**. (c) **Every film frame and product shot built on that render is wrong in the same way**, including the whole 2026-09-09 orientation series and the Kit 2 film's closing shots. **One assumed value, made once because a supplier file had no crease layer, is now wrong in the print proofs, the pack renders, the shot list and the film.** ⛔ **Nothing should be re-rendered or re-shot until the sleeve front is re-laid-out at 169 x 122**, or the work is thrown away twice. **This is a decision sweep, not a packaging edit.**
- 🟢 **RESOLVED 2026-09-09, AND NOBODY WAS WRONG: THE 10 mm GAP IS BLEED, NOT ERROR.** Keith supplied the arbiter — `Capillary Test Kit Artwork for Window 169x122x23mm.pdf`, the exact file the red item below has been waiting on since 25 Aug. **Both files are now pulled into `assets/packaging/dieline/`** (that one plus `IVI_Tasso_Kit.pdf`), which closes the "next action on packaging" that had been open for over two weeks. The template gives **sleeve trim 169 x 304 mm**, panels **122 / 23 / 122 / 22**, aperture **118 x 67 mm** with **R5** corners, and carries the note *"Please leave a 5mm artwork bleed around edge of sleeve and inside aperture."* That note reconciles the two figures exactly: **118 − 10 = 108** and **67 − 10 = 57**, which is our number, and **169 + 10 = 179**, which is our trim. **Laura's figures are the DIE (cut) dimensions; ours are the ARTWORK (bleed) dimensions. Two different lines, both correct, measured against each other for a fortnight.** ⚠️ **One loose end before this is called CLOSED**: the same 5 mm bleed should make the artwork sheet **314 mm** tall, and our extraction recorded **304.00 mm**, so width and aperture reconcile and height does not. Either the `.eps` carries horizontal bleed only, or the height extraction needs re-checking — that is the remaining measurement, and it is small. 🔴 **Two corrections land regardless.** The **corner radius is R5**, not the ~3.2 mm assumed off the raster, so any die drawn to 3.0 or 3.5 is wrong. And **the print proofs are drawn to the artwork aperture, not the cut aperture** — whether that is correct depends on whether Mega-Pak expects trim-and-bleed or cut geometry, which is a question for them, not a redraw yet. Full re-measure of the vector geometry from the PDF is owed before anything goes to a printer. ✅ **The aperture is on the BACK panel (Keith, 2026-09-09).** The template's 118 x 67 window sits in one of the two 122 mm panels and that panel is the back; **the front face carries no window**, so the front pack art is correct as drawn and this is settled rather than open. Original assessment kept below for the audit trail:
