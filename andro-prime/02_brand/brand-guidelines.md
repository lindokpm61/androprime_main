...
2. **Simulated "sample report" preview panels wherever they appear as a preview of the real dashboard: the product (kit) pages, the paid-search landing pages (`app/lp/*`), and the homepage hero** — the hero/sample data cards on Kit 1 / Kit 2 / Kit 3, the equivalent illustrative readouts on the landing pages, and the homepage hero sample panel. These are previews of the dashboard, so they speak the same colour language as the real thing: the amber/green a man sees in the preview is exactly what he'll see in his own results. Added 2026-06-13 (Keith): the colour materially lifts the hero and reinforces the data-first promise, and because the panel's whole function is "interpret good vs concerning", it meets the same test that justifies colour in the dashboard. **Extended to landing pages + homepage hero 2026-07-26 (Keith):** these sample panels are the identical "preview equals real" device as a kit-page panel, so the same justification applies. The fence below is unchanged: colour still appears only on the range-bar fills and status dots inside such a panel.
