...
| Customer.io | Email and CRM sequences | **EU-region workspace `219186`** (Belgium data centre). DPA (EU SCCs + UK Addendum) is auto-incorporated into the GB-region contract, and Customer.io is DPF UK-Extension certified, so **no separate IDTA is needed**. _Corrected 2026-08-04, pending Keith's ratification: this row previously read "US-based — requires UK IDTA SCCs in addition to DPA", which contradicted `dpia/phase0-dpia.md` §4/§5. Reconciled in favour of the DPIA, which is the reasoned position. Residual actions there: verify the DPF certification is current and keep a copy of the DPA. See the DPIA's 2026-08-04 finding on Track API routing._ |
| Hetzner (Hetzner Online GmbH) | **Website and application hosting.** Serves `andro-prime.com`; the Next.js container and its logs run here. Receives usage and request data, and holds health data only transiently in memory while a page is rendered. | Server verified **2026-08-21** as `CLOUD-HEL1`, **Helsinki, Finland** (RDAP on the production origin `37.27.250.169`, the IP Customer.io named in its 2026-08-04 routing report). EEA, so UK adequacy regulations cover the transfer and no IDTA or SCCs are required. Hetzner's standard DPA incorporates SCCs for its own onward transfers. **Added 2026-08-21: this processor was missing from the register entirely**, which is how the privacy policy came to name Vercel, who host nothing of ours. |
| Cloudflare | **CDN, DNS and edge security** in front of `andro-prime.com`. Receives usage and request data (IP, user agent, requested URL) and caches served objects. | Confirmed in front **2026-08-21** by response headers (`server: cloudflare`, `cf-ray: ...-LHR`). US-headquartered; UK Addendum to the EU SCCs is incorporated in Cloudflare's standard data processing terms. No health data is served through cacheable objects: results render on authenticated, non-cached routes. **Note:** `CONTEXT.md` §4b records our own measurement of the edge cache retaining objects after origin deletion, so purge is a live obligation on any takedown. **Added 2026-08-21, previously missing.** |
