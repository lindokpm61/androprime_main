...
> **Change note — 2026-08-21 (the hosting layer was missing from this assessment):** Neither this DPIA nor `data-controller-position.md` named **who serves the website**. Both listed Supabase, Stripe, Customer.io and Vitall and stopped there. The omission is what allowed the live privacy policy to name **Vercel**, who host nothing of ours, unchallenged since it was written. Corrected here and in the ROPA: **Hetzner Online GmbH** (application hosting, verified by RDAP as `CLOUD-HEL1`, Helsinki, **Finland**) and **Cloudflare** (CDN, DNS and edge security, confirmed by response headers) are added to §1 and §2. **Coolify is deliberately NOT listed:** it is open-source software we run on our own Hetzner VPS, not a third party receiving data. Both additions are EEA or covered by the UK Addendum, so the transfer risk assessment in §4 is unchanged. The public privacy policy was corrected in the same pass (v1.3).

> **Change note — 2026-07-09 (Supabase data location + DPA confirmed):** The 2026-07-05 audit confirmed the Supabase project holds all data in the **Ireland region** (EU) — earlier drafts left this "to be confirmed" and several other repo docs wrongly stated "EU (Frankfurt)". The **DPA is incorporated via Supabase's standard terms** (<https://supabase.com/legal/dpa>) — there is **no separately signed DPA**. §1 (processor role), §2 (storage table), §4 (transfer risk row) and §5 (outstanding actions) updated below.
...
| Data processor | Supabase | Database hosting (Ireland region, EU). Stores all account and health data. Encrypted at rest. DPA incorporated via Supabase's standard terms (no separately signed DPA). |
| Data processor | Customer.io | Email delivery and CRM. Receives name, email, order data. **Testosterone-derived data:** as of 2026-06-04 the `low_testosterone` flag is sent to CIO ONLY after explicit nurture consent (`api/lowt-nurture/consent`); the raw testosterone value and the borderline flag are never sent. **Energy-marker traits** (`low_vitamin_d`, `low_b12`, `elevated_crp`, raw `crp_level`, `low_ferritin`) were sent unconditionally at result-processing — a broader data-minimisation gap flagged for a separate decision (tied to the supplement-waitlist consent), not covered by the low-T nurture approval. **2026-07-07: all five energy-marker traits are now gated in code on the CA-018 health-processing consent (fail-closed); raw `crp_level` is kept but gated (seq-03a's hs-CRP >10 email branching compares the numeric value). Pending deploy. Gate-vs-documented-lawful-basis: **Keith ratified the gate approach 2026-07-09**; Ewa co-sign pending — this implements the conservative default.** UK GDPR transfer mechanism for any CIO transfer: Customer.io's DPA — which includes the EU SCCs + UK Addendum and is auto-incorporated into every customer contract (GB region) — plus Customer.io's certification under the UK Extension to the EU-US Data Privacy Framework. No separate IDTA needed (see §4/§5). |
| Data processor | Hetzner (Hetzner Online GmbH) | **Website and application hosting.** Runs the Next.js container serving `andro-prime.com`. Receives usage and request data; health data passes through memory transiently during page render and is not stored here. Location verified 2026-08-21 as **Helsinki, Finland** (RDAP `CLOUD-HEL1` on the production origin `37.27.250.169`). EEA, so UK adequacy applies and no IDTA or SCCs are needed. **Added 2026-08-21: this processor was absent from the DPIA.** |
| Data processor | Cloudflare | **CDN, DNS and edge security.** Receives usage and request data (IP, user agent, requested URL) and caches served objects. Confirmed in front of the site 2026-08-21 by response headers. US-headquartered; UK Addendum to the EU SCCs incorporated in its standard data processing terms. Authenticated results routes are not cached, so no biomarker data is held at the edge. **Added 2026-08-21, previously absent.** |
...
| Hetzner (application server) | No persistent customer data. Request logs and transient in-memory render state | Helsinki, Finland (EEA) | In transit; disk encrypted at rest |
| Cloudflare (edge) | Usage and request data; cached public assets only, never authenticated results | Global edge network, requests from the UK served from London | In transit |
...
|---|---|---|---|---|
| **Data breach exposing health results** | Low | High | Encryption at rest and in transit. Database access restricted to two named individuals. Supabase hosted in certified EU data centres (Ireland region). Regular access reviews. | Low |
...
| **Health data transferred outside UK without safeguards** | Medium | Medium | Supabase data is held in the Ireland region (EU); DPA incorporated via Supabase's standard terms (no separately signed DPA). Customer.io (US) transfer is covered by its DPA (EU SCCs + UK Addendum, auto-incorporated into the GB-region contract) + Customer.io's DPF UK-Extension certification — **no separate IDTA needed** (verify DPF cert current, keep a DPA copy). See the dedicated `low_testosterone` transfer row below and §5. | Low |
...
| Supabase data location + DPA | Keith | Done — 2026-07-05 audit confirmed Ireland region (EU); DPA incorporated via Supabase's standard terms (no separately signed DPA) |
...
| Build explicit consent checkbox for health-data processing | Keith | Built 2026-06-23 (Half 1, CA-018) at **checkout** (`CheckoutDetailsForm` → `/api/checkout/kit` → Stripe metadata → webhook stamp); prod migration applied; **deployed** (verified via git ancestry, 2026-07-05 audit) |
| Gate energy-marker CIO traits (`low_vitamin_d`, `low_b12`, `elevated_crp`, `crp_level`, `low_ferritin`) on CA-018 consent — the §4 data-minimisation gap | Keith | **Done — deployed and verified 2026-08-21.** Implemented 2026-07-07 (fail-closed; `crp_level` kept but gated for seq-03a branching), merged to `main` and therefore deployed, and covered by **37 assertions** in `09_website-app/frontend/scripts/test-cio-traits-consent-gate.ts` (in the `npm test` chain), including the fail-closed path where a consent lookup that throws sends no traits. Gate approach **ratified by Keith 2026-07-09**; **only Ewa's co-sign remains.** |
| Backfill: pre-existing customers (and guest accounts created before this checkbox) hold no `health_processing_consent_version`. New design captures consent at their *next* purchase; decide whether any retained pre-existing results need a separate consent touch | Keith | **Nil as at 2026-08-21 — verified against production, not assumed.** `users` = 3, `kit_orders` = 3, `lab_results` = 1, all Keith's own end-to-end run plus test rows. **There is no pre-existing customer to backfill.** Re-check only if this is still open when real customers exist. |
...
*Unsigned as of 2026-07-09 — awaiting signature from Keith Lindo (data controller) and Dr Ewa Lindo (clinical lead). Sign and date below.*
