...
**Live status:** `ACCOUNT_DATA_CONTROLS_ENABLED` set to `true` in Coolify and deployed 2026-07-19 (Keith). Feature is LIVE. **Outstanding live item:** confirm the erasure ops-alert destination (`OPS_ALERT_EMAIL`, default `keith@andro-prime.com`) is monitored, because erasure requests now arrive there and the 30-day SLA runs from receipt. Condition 2 (automated deletion) is unchanged: the retention/deletion policy is now DRAFTED (`03_compliance/deletion-policy/retention-and-deletion-policy.md`) but not signed; the live feature is request-only so this does not block it.
