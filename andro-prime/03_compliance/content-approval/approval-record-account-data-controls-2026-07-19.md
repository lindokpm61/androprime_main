...
**Live status:** `ACCOUNT_DATA_CONTROLS_ENABLED` set to `true` in Coolify and deployed 2026-07-19 (Keith). Feature is LIVE. **Ops-alert destination confirmed 2026-07-19 (Keith):** erasure requests route to `keith@andro-prime.com`, a monitored inbox; the 30-day SLA runs from receipt. F4 operational gate fully cleared. Condition 2 (automated deletion) is unchanged: the retention/deletion policy is now DRAFTED (`03_compliance/deletion-policy/retention-and-deletion-policy.md`) but not signed; the live feature is request-only so this does not block it.
