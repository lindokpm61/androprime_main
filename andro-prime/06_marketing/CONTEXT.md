...
│   ├── 2026-06-24-test-led-positioning-validation-flywheel.md ← test-led doctrine (now the acquisition/content thesis, not the brand lead; see Special Cases)
│   ├── 2026-06-26-feeling-first-content-strategy.md ← feeling-first topic doctrine (lead with the feeling, reveal the test as the answer)
│   ├── 2026-06-26-tier2-sales-creation-plan.md ← ★ Tier-2 sales creation plan (the doc the v2.2 corpus was SUPERSEDED into)
...
├── content/                              ← Founder + social asset home (no longer just a single skill-context file)
│   ├── copy-content-context.md           ← Skill context: copywriting, copy-editing, social-content, cold-email
│   ├── social-channel-setup.md           ← Founder account setup, handles, bios, channel architecture
│   ├── youtube-founder-journey-strategy.md ← Founder YouTube lines (long-form + journey)
│   ├── track-a-launch-copy.md            ← Founder launch copy (Track A)
│   ├── 2026-07-09-youtube-channel-naming.md ← YouTube channel naming decision
│   ├── email/newsletter/                 ← Newsletter issues + newsletter-playbook.md
│   ├── instagram/templates/              ← IG story/reel/feed templates (PNG) + README
│   ├── linkedin/                         ← Keith launch posts
│   ├── reddit/                           ← Reddit assets
│   ├── youtube-scripts/                  ← Example founder video scripts
...
└── seo-ai-search/                        ← THE CONTENT ENGINE
    ...
    ├── keywords.csv                      ← DFS single-source master (20-col: priority, serp_verdict, kd_source, coverage_status). 1,055 rows. See seo-data-rebuild-build-doc.md
    ...
    ├── keyword-expansion-shortlist.md    ← Keyword expansion shortlist (+ keyword-expansion-staging.csv)
    ├── 2026-07-20-keyword-backfill.md    ← Keyword backfill pass (2026-07-20)
    ├── vitall-keyword-to-kit-map-sized-2026-07-19.csv · vitall-kit-keyword-brief-2026-07-19.md ← Vitall keyword→kit map (sized) + brief
    ├── content-strategy-tier1-results-decode-2026-07-14.md ← Tier-1 results-decode content strategy
    ├── voc-reddit-quora-2026-07-14.md    ← Voice-of-customer mining (Reddit + Quora)
    ├── geo-third-party-presence-outreach.md ← GEO third-party presence + outreach plan
    ├── partner-proxy-buyer-persona.md    ← Partner-proxy buyer persona
...
**Content CTAs never route to the founding-member list:** Article/blog/LP/email/ad CTAs go to **Kit 1 / Kit 2 / Kit 3 / supplement waitlist**: **never** the FM list, and never "priority access to TRT" / "clinical service waitlist". Driving un-tested content readers into an opt-in for a future regulated (TRT) service is the CQC/ASA recruiting-undocumented-subscriber pattern. Note: FM is now even further off-limits: acquisition pivoted to products+supplements and the FM list was **taken down** entirely (low-T results route to GP referral, not FM; see `04_products/CONTEXT.md`). Verification grep before any content ships: `founding.member`, `FM list`, `join the list`, `priority access to TRT`, `clinical service waitlist` → remove/reframe any hit. Per-pillar primary CTA: C→Kit 1; A→Kit 2/Daily Stack; G→Kit 2/Collagen waitlist; D→Kit 2 or Kit 3; E (andropause, cleared CA-028 2026-07-26)→**Kit 1 or Kit 3**, per CA-028 §5, and never the FM list. (This previously read "Kit 3 / Kit 3 Plus"; Kit 3 Plus is not launched, and the signed pack names Kit 1 and Kit 3.)
...
> ⚠️ **NOT THE BRAND LEAD ANY MORE (2026-07-22).** Keith adopted the **conflict-free positioning** as the governing brand position (`../01_strategy/2026-07-22-conflict-free-positioning-decision.md`; approved wording **CA-026**, `../02_brand/2026-07-22-conflict-free-wording-pack.md`). Test-led / "don't guess, test" is retained as the **acquisition and content thesis** that sits beneath the conflict-free lead, not as the customer-facing headline. Customer copy leads speed / no-GP first, money honesty second (one price; any result that needs a doctor goes to a GP and earns us nothing; no result changes what we offer or what it costs). The sharp line "no reason to sell you testosterone" is press/GEO/comparison only, never on-site copy; on-site press form is E2. Do not use "we earn the same whether your results are fine or not" as a live claim.

**Content audience-tier mix (tracker #31):** target ~40% wellness / ~40% clinical-curious / ~20% TRT-specific. **Pillar E and sexual function have split (2026-07-30) and must no longer be treated as one gated block.** Pillar E cleared (CA-028 approved 2026-07-26, hub live 2026-07-30) and should now be planned in deliberately; sexual function stays HELD for Ewa; TRT itself stays at ~0%, which is the Phase 0 boundary rather than a sign-off question. Every Pillar E asset still needs its own pre-flight plus Ewa's own sight at draft. The pillar set structurally skews clinical-curious (D/H/I/J are the biggest, lowest-compliance gaps), so the operative near-term rule is **protect a ~40% wellness floor**: interleave a wellness pillar (A/B/Omega-3) for roughly every clinical-curious one. Map lives in `seo-ai-search/content-calendar.md`.
...
- Founder content + blog: `/article`, `/article-to-review`, `/publish-article`, `/content-status`, `/hook`, `/script`.
- Copy compliance: `/compliance-preflight`, `/draft-preflight` (run before any external-facing copy ships).
- SEO / AEO: `ai-seo`, `seo-audit`, `programmatic-seo`, `schema-markup`, `competitor-alternatives`.
...
- **dataforseo** (MCP, wired): the core SEO toolset: keyword volume/ideas, SERP, backlinks, DFS Labs, trends, AI-optimization/LLM-mentions. Backs the `seo-ai-search/` dated CSV artifacts.
- **reddit** (MCP, connector): VOC and community research (the reddit/quora VOC docs).
- **Semrush** (MCP, connector): additional SEO/competitor data. Needs authorising before use.
- **Figma** (MCP, connector): creative and social asset design. Needs authorising before use.
- **Customer.io** (MCP, connector): newsletter/broadcast sends (lifecycle detail lives in `07_sales`).
- **context7** (MCP, wired): current library/API docs when building content tooling.
