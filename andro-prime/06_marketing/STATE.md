...
- ⚠️ **Live-versus-mirror drift, confirmed by accident and NOT fully assessed.** The `free-androgen-index` MDX mirror in `seo-ai-search/article-drafts/` still carried the **pre-K1** FAI wording while the live row had the corrected framing. It surfaced only because a string replace failed to match. That mirror is realigned; **the other eight carousel articles were not diffed**, and the same drift on any of them would mean the repo copy is not what a reader sees.
