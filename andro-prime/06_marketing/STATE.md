...
- **None of the eight sets `coverPhoto`**, deliberately, because their newspaper frames are not inpainted. The covers render the missing-photo hatch. Pointing them at the default would put each topic's title on a plate over the vitamin D newspaper, which is exactly the fallback removed from `build.js` earlier the same day.
...
- **🔴 THE B12 BAND ON THE SLIDE WAS WRONG, AND SO IS THE LIVE ARTICLE. Both corrected.** The deck said "under 35 is low", inherited verbatim from `content/blog/b12-blood-test.mdx`, which attributes it to *"the bands Andro Prime's Kit 2 reads against"*. `classifier.ts:325` reads `value < 25` on the NICE NG239 three-band scheme **Ewa signed 2026-06-16 and re-ratified 2026-08-07**. The article published 2026-06-20, four days after that decision, and was never swept. The deck and the repo's MDX mirror now read `<25 / 25–70 / >70` and cite NG239. **No fresh sign-off sought**: this applies her decided value, on the precedent of the 2026-07-25 `6 → 12` GP-threshold fix.
  ...
  ⚠️ **THE LIVE PAGE IS STILL WRONG, and a deploy will not fix it.** Verified 2026-08-11 after pushing `0a27d77`: `https://andro-prime.com/blog/b12-blood-test` still serves *"under 35 pmol/L is low"*. Article bodies are served from `blog_articles.body`; `content/blog/*.mdx` is a **lagging mirror** that the site never reads. An earlier version of this entry said the fix "needs deploying", which was wrong and is retracted. It needs a `blog_articles.body` update (two occurrences: the FAQ answer in `faq` and the body paragraph under "What counts as low"), a `/api/revalidate` call for the slug, then a two-sided check on the served page. Take a backup first, per the 2026-07-31 precedent. **A published page is currently misstating our own Ewa-signed threshold and attributing it to Kit 2.**
