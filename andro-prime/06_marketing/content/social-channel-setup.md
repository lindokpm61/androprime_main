...
- **Open risk, DOWNGRADED 2026-09-09 (Keith):** the whole company brand, including X, LinkedIn, TikTok and YouTube, is now pointed at an Instagram account that is days old. The "account history of repeated restrictions" this line used to cite as the risk **was a device problem, not a brand, product or content problem** (see the account-creation note below), so it does not carry forward to the live account. What remains is the ordinary risk of a young account, not a flagged one. Hand-posting to that IG for the first fortnight and connecting Metricool to it later was recommended and is **not yet decided**.
...
- **Account-creation note, CAUSE RECORDED 2026-09-09 (Keith).** The "create it from inside the established account" advice below **was tried and failed**: accounts created that way were restricted then banned within hours, empty. **The cause was the phone used to create them, not Andro Prime.** Keith confirmed 2026-09-09 that the deactivations tracked the device, and had nothing to do with the company, the brand, the products or the content. **This matters well beyond the handle question: with no cause on the record, a later reader attributes the bans to the most salient nearby explanation, which is that we sell a regulated health product. That inference is wrong, and it was drawn once, on 2026-09-09, inside a paid-social feasibility answer.** Instagram is not treating this brand as a policy problem, and nothing here ever said it did. Still unrecorded: exactly which route or device produced the surviving `@keithandroprime`. Capture it if it is ever needed again, but the answer is now known to live in the device, not the positioning.

**Instagram handle change (2026-07-19), SUPERSEDED by the 2026-08-09/10 block above:** the `@keith.androprime` handle above is **superseded**. Instagram kept auto-deactivating freshly created accounts (cause since established as the device used to create them, see the note above), so the founder presence moves to an **existing, established account: `@keith.antony.tech`**. The handle is a non-ideal fallback (chosen over `@keith.antony.ai`, because "ai" in a health handle wrongly signals AI-generated content and undermines trust); the **branding is carried by the profile Name field and bio, not the handle**. When creating the `@androprime` placeholder, create it from inside the established account (not a cold signup) so it survives the same deactivation pattern.
