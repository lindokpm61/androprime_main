...
**Owed, and not built (recorded so it is not rediscovered):** (1) a `uuid` column on `content_renditions` that `metricool-schedule` captures, so the join survives a reschedule instead of orphaning until someone re-maps by slot; (2) `content-doctor` I3 should report an unresolvable id as **"id rotated, re-map by slot"** rather than as a dead post, because the post is almost always alive under a new number and the current wording sends the reader hunting for a deletion that never happened. The general lesson from how this rule was first written: **document the MECHANISM, not the ceremony you happened to observe it during.** "Arming replaces the id" and "any write replaces the id" look equally specific and only one is true, and the narrower version is worse than no rule at all, because it certifies the untested path by omission. Before writing such a rule down, ask what the system is actually doing at the moment of the change. (Observation 301.)

🔴 **Two more tooling findings, recorded because the code fix is not written and the next reader will otherwise rediscover them.**

**(a) The `route_verified_at` guard's documented discharge path is wrong.** Two scripts disagree about which one sets the guard's input, and the flip script's header names the migration file as the thing that clears it. A migration is a poor home for a statement that has to be re-run periodically, because "migration" tells every reader it already ran. **Preferred fix: make `metricool-writeback` own the update** — it already has the evidence in hand at the moment it records a publish, so the guard would satisfy itself as a side effect of truth arriving. Failing that, correct the flip script's header and drop the write-back clause. The general lesson: **a guard is only as usable as its documented discharge path**, and when script A states how to satisfy its own precondition by naming script B, that sentence is an **untested integration claim sitting in a comment**. It fails in the most expensive direction — the reader runs B, sees it succeed, and concludes the guard is broken rather than the instruction. Walk any "run X and this clears" instruction once before writing it down. (Observation 302.)

**(b) `content-doctor` I7 passed while the newest section was stale.** I7 asserts that counts quoted in STATE docs match the database, and it reported clean because it matched **zero assertions** in the newest dated section, which renders identically to "all current assertions match". Owed: a fixture in `test-content-doctor.ts` built from the CURRENT top-of-file format of `content-machine/STATE.md` (a bolded run-on sentence carrying six figures), asserting I7 fails when any figure is wrong; and I7 should report **how many assertions it found in the newest dated section**, because "0 current assertions in the newest section" is itself a finding. This is `cross-cutting-principles.md` **P28** in a text matcher rather than an instrument. (Observation 303.)
