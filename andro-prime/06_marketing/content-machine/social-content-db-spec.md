...
> **Sections 1 to 7 are the PROPOSAL as it stood on 2026-07-28 and are deliberately not rewritten; section 8 is the record of what was built.** Where they describe frontmatter as the live store or `scan.js` as the gate scanner, that is the state being argued against, not the state today: Phase 1 (2026-08-01) moved every state field into `content_assets` / `content_renditions` and every gate into database constraints. Read section 8 and `STATE.md` for current truth.
...
- **Decision 1, DB-as-truth or projection: DB-as-truth.** Follows the Spine A precedent rather than inventing a third pattern. As written on 2026-07-28 the git `assets/*.md` files remained and became a mirror while the gate scanner still read them, so **there was a transition window where both existed and the JS scanner was the one enforcing git**, and closing it was named as the next piece of work. **CLOSED 2026-08-01 by Phase 1, exactly as described.** State left frontmatter entirely; the file now owns identity and craft only; `scan.js` was repointed to what a repo-only reader can honestly verify (schema, YAML safety, compliance, and a HARD failure on any database-owned key that comes back) and its gates G1 to G4 moved into database constraints; `content-sync` regenerates a read-only state block into each file from the database. The 2026-07-28 warning that this window would drift was correct: it drifted within four days, in `content-library-sync.ts`. See `STATE.md`, "Phase 1 is BUILT".
...
- **Decision 3, canonical FK as hard gate: hard gate, on by default.** Implemented in `gate_rendition_publish()`. It guards the single rule the whole atomisation model rests on. To downgrade to advisory: `drop trigger gate_rendition_publish on public.content_renditions;` **Still true 2026-08-01**, and the trigger now also fires on `INSERT` (see 8.3). Its definition of record is `09_website-app/database/migrations/20260801_content_state_guards.sql`; before that date the gates existed only in the live database, with no migration file, which is how a later piece of work nearly added a second parallel copy of them.
...
- ~~**Gate triggers fire on `UPDATE` only, never `INSERT`.**~~ **SUPERSEDED 2026-08-01 (Phase 1).** The original reason was sound and is worth keeping: backfill had to record history as it actually happened, including the one asset that shipped ahead of its gate, rather than rewriting it into something the rules would have allowed. **But the backfill finished on 2026-07-28, and after it the exemption stopped being an affordance and became a hole:** a gate you can arrive at without passing through is not a gate, and every one of these tables is written by scripts that insert. `gate_rendition_publish` now fires on `INSERT OR UPDATE`, and the asset approval gate became a CHECK constraint, which is evaluated on every write by construction. **If a future backfill needs the old behaviour, disable the trigger for that transaction explicitly** (`alter table content_renditions disable trigger gate_rendition_publish;` … `enable`), so the exemption is a visible, deliberate act rather than a standing property of the schema.
...
- **`instrumentation-problem` is a live post on an asset at `scripted` with `preflight: amber-ewa`.** The DB now holds that contradiction explicitly rather than as a scanner warning, and the approval gate refuses to resolve it until ~~an `ewa_task` exists~~ **Ewa's ruling is recorded** or the pre-flight goes green. Verified by attempting the update and being refused. **Tightened 2026-08-01:** an `ewa_task` merely proves the question was ASKED. The gate is now `ewa_signed_at`, written only by the sign-off sync that reads her ClickUp task, per Keith's reconciliation of the same rule in `content-doctor` invariant 5.
