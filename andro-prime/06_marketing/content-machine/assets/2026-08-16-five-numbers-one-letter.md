---
slug: five-numbers-one-letter
title: Five numbers, one letter (what a cholesterol panel actually says)
content_type: educational
funnel_stage: MOFU
funnel_job: panel breakdown (cholesterol / ApoB) for the older segment, route to the list
...
Archetype: Teacher, calm and plain, aimed at a man holding the printout rather than at the scroll.

Line 1 names the gap between what the result contains and what the covering letter explained. No punch, no wedge: the Facebook reader is older, patient and clinical-curious, and the job is plain-English education that stands on its own value.
...
>
> Triglycerides are a different blood fat, your body's main way of storing spare energy. They climb with excess calories, alcohol and refined carbohydrate, and they jump after a meal, which is why the target for them is a fasting one.
>
...
**Why that split matches the two audiences.** LinkedIn is Keith's personal profile and rewards a POV. The Facebook reader is the older segment, more clinical-curious and patient, and less driven by a scroll-stop, so the value has to be in the post itself rather than in the turn at the end. A man reading this can hold his own result beside it and follow line by line, which is a job the LinkedIn version deliberately does not do.

**Shape check against the library** (`/script` Step 4b). Facebook closes converge hardest of any surface: three of the four live Facebook posts end on an `If you [want / are not sure]…` conditional plus a link, and one of those three is `the-stack`, where the form was carried in earlier today. This one closes on a statement of what is and isn't available, then the link. Openers avoid `Most men…` (three in the library), `If you are…` (two) and `A man I know…` (taken by `the-stack`).

**No closing question, on purpose.** The mandated reader-question is a LinkedIn rule, where it earns a comment. All four live Facebook posts end on the link instead, and this follows them.
...
- **`thumb: none` follows current practice, not the `/script` fan-out table**, which says Facebook takes 1200x630. All four live Facebook renditions carry `none`, and `metricool-schedule.ts` attaches the canonical article's own photo on Facebook automatically. Worth reconciling the skill table with what the scheduler actually does.
