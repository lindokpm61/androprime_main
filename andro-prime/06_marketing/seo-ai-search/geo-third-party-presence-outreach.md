...
| **11** | **bloodtests.head-to-head.uk** | 🟢 **NEW, and the best route in the whole workstream.** Independent compare hub (Medichecks, Thriva, BetterYou, Randox, Bluecrest, Forth, Numan). Cited on `best private blood test UK`. | ✅ **`/contact` form with an explicit "Suggest a Provider" and "Featured Provider Request" subject**, and copy that asks *"want to get featured on our site?"*. An **invited** route, not an inferred one. No honeypot, no captcha. | 🟢 **LISTED AND VERIFIED 2026-09-06.** Submitted 2026-08-15 (CA-037, `/contact` form, Subject "Suggest a Provider"); Adam Groves replied 2026-08-18. **Verified by fetching the page, not taken on his word**: `https://bloodtests.head-to-head.uk/providers/androprime/` returns 200 with **four `rel="noopener"` links and NO `nofollow`** (homepage plus all three kits), and we appear on their homepage. **Already earning**: `bloodtests.head-to-head.uk` shows up as a cited source in Perplexity on `best private blood test UK` in the 2026-09-06 sweep. Not yet in DataForSEO's backlink index, so it is not among the domain's 12 known links. **Still owed: reply to Adam** (he asked whether we run an affiliate programme; needs `/compliance-preflight` first, and the recommendation on file is to decline). |
