...
### 21a. Voice revision, 2026-07-30 (rev3): prose only, still awaiting the same Ewa gate

_The 26 July record above stands as written; it documents rev1. This block documents the in-place rewrite that replaced it while the article was still unopened in Ewa's queue._

**Why.** The draft was committed 2026-07-27 01:30 (`2a1d82f`). The voice standard changed 22 hours later in `b19ea79`: tone-of-voice v1.1 → v1.2 added §9a (six machine-writing tells, each a hard fail, plus four carve-outs protecting house devices), `02_brand/references/narrative-devices.md` was created (nine devices from the spoken corpus), and `/article` moved from a 13-point to a 14-point bar and now loads narrative-devices. This was the last article drafted to the old standard. Keith's call, 2026-07-30: rewrite now rather than let Ewa sign rev1 and then revise a signed Pillar E article, which is the position `myth-of-normal-range.mdx` is already stuck in.

**Scope: body prose only, verified.** Diffed against the committed rev2: all headings byte-identical, all eight FAQ q/a pairs byte-identical, every citation and URL byte-identical, the `{/* TODO Ewa sign-off */}` PullQuote block byte-identical, the `<EvidenceBox>` block byte-identical, the References block byte-identical, all internal links and both CTA panels byte-identical. Frontmatter unchanged except `dateModified` 2026-07-26 → 2026-07-30. One wording correction outside the prose: two instances of "UKAS-accredited lab" → "UKAS ISO 15189-accredited lab", the substantiated form (`seo-content-context.md` §8).

**§9a fixes applied.** Removed four throat-clearing / meta-joiner openers ("This is the search everyone eventually types", "Two things make a testosterone test trustworthy", "If you've read this far", "It comes down to one choice"). Cut the forced per-H2 diagnostic question under the "is it real" H2 (the H2 already asked it; §9 box 2 is explicitly not a per-section quota in long-form). Cut six of eight two-clause epigrams, keeping the two load-bearing Move 4 reframes ("It isn't that the male menopause is a myth. It's that…" and "You don't have a symptom problem. You have a 'which cause is it' problem"); at eight, the reframe had become the formula it exists to avoid. Fixed the undeclared third-person to second-person POV switch with a deliberate handoff (device 9: start inside one experience, earn the generalisation, spend it once). Personification hits were run through the §9a named-actor test and **kept**: "a checklist names what you're feeling", "a checklist was never built to solve" are load-bearing and mirror the approved reference-range construction.

**Narrative devices added.** Device 1 (ordinary build, then rupture): the opener now builds the mundane Tuesday properly and breaks it flat in one sentence. Device 6 (banal scene carries the load): the car park at half two, the stairs he couldn't face. Device 3 (physical object for an invisible state): "driving with the handbrake half on", a feeling description with no mechanism or outcome claim, pre-flighted clean. Device 4 (show the search for the word): one visible hesitation about naming, which is the article's own subject ("I'm not sure andropause is a much better word, honestly"). Device 8 (flat close): the closing reader-question was replaced with an open wondering that returns to the opening scene, which §9 box 7 now accepts in long-form and which removes the uniform faintly-salesy ending.

**Gates re-run.** 2,486 words (band 2,200-2,600). Voice: 14/14, plus the §9a pass clean. `compliance-preflight`: **3 🔴 HARD / 0 🟠 REVIEW / 0 🟡, identical to rev1**: the same three intentional query-echo uses (H2 + FAQ Q6 "treatment", FAQ Q7 "diagnose"), at lines 196, 52 and 60. **The rewrite introduced zero new flags.** 0 em dashes, 0 banned voice-off words, 0 "you should / you need to". Compile-gate PASS against the prod preview render path. Rendered preview fetched and asserted: new prose present, removed lines absent, PullQuote / 12 nmol/L GP routing / no-diagnose-no-prescribe all still on the page.

**Pipeline state.** `blog_articles` row unchanged and still `status='draft'`; rev3 `3048fabc` written by `draft-writer` and now `current_revision_id`. Nothing deleted: rev1 `1fc02f26` and rev2 `ef2a7e66` are retained. `content_pipeline` back at `stage='in_review', blocked_on='ewa'` on the same row. **No second ClickUp task created.** Task `869e9hey1` is the single open submission and its preview link serves rev3 automatically, because the preview renders from the DB. `content_review_log` row `7c49693d` repinned from rev1 to rev3, with the revision history in its `notes` (it was already stale, pinned to rev1 while rev2 was live). Ewa commented on 2026-07-30 with what changed, what did not, and the same three rulings restated.

**Unchanged from the list above:** open items (1) through (5) all still stand. Ewa's written sight on the full draft remains the gate, and none of this is approval.
