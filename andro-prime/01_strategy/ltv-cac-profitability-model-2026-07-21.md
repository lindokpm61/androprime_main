...
- **Prices are working hypotheses**, accepted 2026-07-21 to unblock build + modelling; the Van Westendorp read can move them. Kit COGS are implied from v1 contributions, pending any Vitall second-order/wholesale confirmation. ~~(also confirm order-time vs dispatch-time separation and kit shelf life, owed before B1 builds)~~ **BOTH CLOSED 2026-07-22; swept here 2026-08-04.** The deferred second-kit dispatch is **our** mechanism, not Vitall's: we place each order ourselves via their createOrder API at trigger time, so order-time vs dispatch-time separation is resolved by design and **kit shelf life is moot** (the second kit is never pre-bought or held). Keith direction recorded on ClickUp [`869e74vwz`](https://app.clickup.com/t/869e74vwz). Billing timing is independently corroborated by Schedule 1 §1.3 / §4.4 (priced per *completed* order) and Ben's 2026-07-21 email ("chargeable once analysed").
