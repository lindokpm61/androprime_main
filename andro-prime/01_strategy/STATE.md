...
_Last updated: 2026-09-17 (✅ **TWO WORDING RULINGS, BOTH KEITH, BOTH APPLIED AND SWEPT THE SAME DAY.** **F4, the cancel clause: "Cancel anytime".** The 2026-09-11 draft said *"unless you stop it first"* while `lib/membership/disclosure.ts` said *"Cancel anytime"*, and both rendered on `/kits` and all three kit LPs within one screen of each other. Ruled for the wording the 2026-09-07 auto-renew ruling already specified; recorded in `2026-09-16-membership-is-not-a-subscription.md` §8. Three strings in `subscriptionCopy.ts` moved, plus two on `/membership`, **plus `RENEWAL_CLAIMS` in the interlock, which had to move with the copy or it would have gone on guarding a string nothing produces.** **F1, the obligation follows the buy button.** The disclosure was mounted once per route, in the hero, and each `/kits/*` page ends on a SECOND buy button whose footnote had stopped mentioning billing at all after slot 2's deletion. Six mounts added. **§4 of `2026-09-07-auto-renew-at-day-30.md` now states its scope as a RULE (every kit buy button, wherever it is) rather than as a list of routes**, which is what let one-per-route read as compliant. Both verified as rendered with the flag on and verified rendering nothing with it off. ⚠ **Neither discharges the independent pre-flight**, which returned 3 HARD and 6 flagged against this copy; closing the items it raised is not re-running it. Earlier the same day: ✅ **A TESTOSTERONE RESULT UNDER 12 ROUTES TO THE GP AND STARTS NO MEMBERSHIP** (Keith, 2026-09-17). Doc: `2026-09-17-a-low-result-starts-no-membership.md`, built the same day as the first guard in `lib/membership/startOnResult.ts`, 37 assertions, mutation-verified. Nothing is sent, offered or charged; a man who returns pays full retail £99 and his 30 days start from that result. Borderline 12 to under 15 is untouched, and a null reading is not a low one. 🔴 **IT IS WORTH £58.50 A HEAD**: without it the auto-renew ruling turns a low-T customer from +£38.02 to −£20.47 — £96.53 net against two kits at £58.50 COGS — and **he never pays the £47**, because the confirmation lands inside the included 30 days and he cancels on day 29. 🔴 **A REFUSED OPTION IS RECORDED RATHER THAN DROPPED**: Keith proposed a ~£69.99 coupon emailed on a low result, with the membership starting from the recheck. The arithmetic was sound (+£47.76) and **it is forbidden by his own rule ADOPTED ten days earlier** — `2026-09-07-fast-recheck-must-be-prepaid-or-included.md`: a result-triggered recheck under 90 days *“must be prepaid or included… it may never trigger a new sale”* — and by **Ewa’s CA-014** (no kit upsell on a low-T result), which is not Keith’s to move alone. That rule permits exactly two doors, and both were walked into independently: *included* is what the arithmetic kills, *prepaid* is the bundle. **So the Recheck bundle is shaped as a prepayment precisely so the sale happens before the bad news**, which inverts how it was read earlier in the session. 🟠 **BUNDLES: Prove-It (£199) and Full-picture (£259) RETIRE** — both sell a day-90 retest the membership already includes, which is worse than redundant. **Recheck (£169) stays in principle**, not yet executed, and `BUNDLES_ENABLED` must SPLIT first because it gates all three plus the clinical trigger as one unit. All three are already dark (flag unset), so this changes code and documents, not the live experience. Earlier: 🟠 **KEITH CLARIFIED THAT THE SUBSCRIPTION IS THE SUPPLEMENT PRODUCT, NOT THE MEMBERSHIP, AND IT IS A FACT ABOUT THE PRODUCT RATHER THAN A PREFERENCE.** Doc: `2026-09-16-membership-is-not-a-subscription.md`, DRAFT, three questions await his ruling. “Subscription” has meant Daily Stack / Collagen / Complete Men's Stack since the v7 catalogue; that product was deferred out of Phase 0a on 2026-05-23 and the Stripe sub route returns a clean 400 by design, so nobody can buy one and no customer holds one. Membership is the separate GBP 47/month product that auto-renews on day 31. **The proposed rule: customer-facing copy says MEMBERSHIP, and “subscription” is reserved for the supplement product** — reserved rather than retired, because `04_products/CONTEXT.md` still schedules Phase 0b reinstatement, which is also the argument against renaming the database or the route. 🔴 **WHAT IT EXPOSES: the account surface is named after the one product nobody can buy.** `/subscriptions` renders the title “Your Subscriptions”, the nav item “Subscriptions” and the empty state “You do not have an active subscription”, and it is where a member is sent to cancel his membership. **Labels only** — the lookup half was already fixed (`BillingPortalButton` 2026-09-11, `checkout/portal` and `getSubscriptions` 2026-09-13, both now reading `memberships` and `supplement_subscriptions`), and an earlier read in the 2026-09-16 session repeated the 2026-09-11 draft's stale §7 saying otherwise. 🔴 **WHAT IT DOES NOT FIX: row 42a does not shrink.** Splitting the live `verify-subscription-claims.js` output, **only 6 of the 18 flagged lines contain the word “subscription” at all**; the other 12 say “One-off purchase”, “Pay once”, “all-in, one-off” or “One-off test”, which are claims about the money, not the noun. ⚠ **AND THE COUNT OF RECORD IS STALE EVERYWHERE: row 42a, the `P6` go-live gate and the script's own header all say THIRTEEN sentences on seven surfaces; the script now reports 16 on 7 pages plus 2 non-page surfaces**, because `CLAIMS` was widened on 2026-09-15 with `pay once`, `one-off test` and `all-in, one-off` and nothing updated the record. **P6 is eighteen sentences.** 🔴 **The open question that is genuinely Keith's: `03_compliance/terms-and-conditions.md:308` says *“Andro Prime Membership is an ongoing monthly subscription”*** — v1.3, drafted 2026-09-13, and that wording is what pulls the 30-day price-change notice and the cancellation duty onto membership. Recommendation is to KEEP it and add one mapping sentence, since stripping it re-opens those protections for no commercial gain. The DMCCA scope question (gap doc residual (b)) is about substance and a naming rule does not touch it. Earlier: 🔴 **THIS FILE HAS BEEN WRONG ABOUT THE FIRST MONTH FOR TEN DAYS, AND IT IS THE STALE-DOC PATTERN THE ROOT CLAUDE.md WARNS ABOUT.** The text below still lists "whether the first month of membership is included in the kit price" as STILL OPEN. **It was decided on 2026-08-27** and recorded in `08_customer-journey/journey-spine.md` stage 9 and `07_sales/funnel/site-funnel-model.md` §4 (commit `d695683`): **the first 30 days are included in the kit price**, so stage 9 stops being "will you start paying £47" and becomes "you are already a member until [date], do you want to carry on". The decision was swept into 07_sales and 08_customer-journey and never swept back here or into `2026-08-26-membership-offer-window.md` §6, so both still assert the opposite. Keith caught it when the assistant quoted the stale text back to him. ✅ **SWEPT 2026-09-06.** The ruling now has a canonical record at `2026-08-27-first-month-included-in-kit-price.md`, and the section below is its home in this file. ✅ **BOTH FINANCIAL MODELS WERE RERUN AT GBP 47 ON 2026-09-07.** They were built at GBP 49, a placeholder that predated Keith's 2026-08-24 GBP 47 ruling and was never a decision. The year-1 forecast had been **contradicting itself**: §1 modelled 49 while §6 recorded 47 as decided, in the same file, since the day it was written. Reruns were validated by first reproducing every published figure at 49 from the stated assumptions. **Year 1: revenue GBP 54,519, profit GBP 30,468, margin 56%** (was 55,362 / 31,297 / 57%). **90 days at twenty members: GBP 793** (was 852), and **the GBP 40 CAC row changes sign, from +51.60 to −7.25** — at this scale a two-pound price difference is the whole margin. Two things flagged rather than changed: the 90-day model's conversion-sensitivity pair does not reproduce from its own method and did not before the price change either; and the anchor ruling may cut "3 payments in 90 days" to 2, which would take the twenty-member case to about GBP 323. ✅ **BOTH MODELS WERE THEN RERUN AGAIN THE SAME DAY ACROSS ALL THREE KITS** (Keith: *"run the forecast on all the kits at 47"*), because the included month attaches to EVERY kit buyer and both models had been using **Kit 2 alone as a proxy for every kit sold**. Blended at the documented 68 / 148 / 38 mix: **price GBP 122.62, COGS GBP 67.03**. **Year 1: revenue GBP 55,578, profit GBP 30,286, margin 54%** (was 54,519 / 30,468 / 56%) — revenue UP GBP 1,059, profit DOWN GBP 182, because the blended gross per kit (GBP 55.59) lands within 41p of Kit 2's (GBP 56.00): Kit 1 drags and Kit 3 lifts by almost the same amount. **The old proxy was accidentally near-perfect on profit and wrong on revenue and margin**, which matters because revenue is what the VAT threshold counts: **the strong case moves to GBP 111,157, GBP 2,115 further over the GBP 90,000 line.** **90 days at twenty members: GBP 740** (was 793), and the **GBP 40 CAC row goes from −7.25 to −60.06, break-even CAC tightening from about GBP 40 to about GBP 37.** The two models move in OPPOSITE directions on the mix and the reason is the retest: over 90 days it is bought at blended COGS and never sold, so a richer mix adds cost with no matching revenue, while over a year the kit line's extra revenue offsets it. **Mix is now a load-bearing guess** and is sensitivity-tabled in the forecast's new §3a: profit moves about GBP 3,900 across the whole plausible mix range, against GBP 52,000 for the acquisition ramp, so **the model's standing conclusion survives — how many kits you sell still dominates everything.** Also flagged in the forecast, not fixed: §6's "roughly GBP 18,500 off the strong case" is computed on TOTAL revenue, not on the membership line as its own sentence claims (20% of the strong case's subscription is about GBP 6,600 if GBP 47 is VAT-inclusive), and which is right depends on the supply characterisation still owed to the accountant. 🔴 **BOTH RERUNS EXIST ONLY ON `redesign/direction-f`.** `main` still carries the GBP 49, Kit-2-only version of the year-1 forecast, last touched in `803cb4c` on 2026-08-25, so the model on main is now two reruns stale. ⚠️ **£47 is also mis-stated as open here.** It is ADOPTED in the build with a stated rationale -- `lib/subscriptions/products.ts`: "£47/month, adopted for VAT-threshold stability rather than for revenue" (Keith, 2026-08-26). What is open is only whether WTP data could reopen it. Adopted-on-VAT-grounds and unresolved-pending-evidence are different statuses and this file conflates them. ✅ **DECIDED 2026-09-07 (Keith): EVERY DATE ANCHORS TO THE RESULT LANDING.** Doc: `2026-09-07-anchor-everything-to-the-result.md`. Four of the eight retest mechanisms already complied; the membership and the timed bundles move; seq-04 e5 is the one carve-out (it measures supplement effect, so its clock legitimately starts at `subscription_started`). 🔴 **A NEW QUESTION FALLS OUT AND IS NOT DECIDED: what happens to a timed bundle whose customer never posts a sample.** Today the prepaid second kit ships at purchase + 90 regardless; under the ruling `due_at` stays null and it would never ship. ✅ **DECIDED the same day: the purchase + 180 backstop.** `due_at` is stamped purchase + 180 at checkout and the result hook overwrites it to `min(result + 90, purchase + 180)`, so the backstop is a ceiling rather than a target and a late result can never push a guaranteed dispatch outward. One column, one write, no migration. ✅ **RESOLVED SAME DAY: the timed bundles are EXEMPT and stay at purchase + 90** (Keith, "purchase + 90", which is arithmetically the exemption). The sweep had found `../03_compliance/terms-and-conditions.md` §"Test Bundles", approved by Keith and Ewa 2026-07-25 and live on `/terms` since 2026-07-26, promising timed-bundle retests *"about 90 days after your purchase"*; no result-anchored date can satisfy that. **The escalation closed with NO change to approved copy, no Ewa re-approval, and no code change on the bundle side.** The 90-day spacing is the product for Prove-It and Full-Picture, not a clinical cadence. **Net effect: the ruling's only code carrier is `createMembership`.** The question this closed was: Journey-spine stage 9 names the collision (the included month would start at PURCHASE, the offer window starts when the RESULT lands, and a slow lab puts them out of step). The code today has a third anchor: `createMembership` stamps `started_at` and the retest date from the moment of STRIPE CHECKOUT, while `offerState` keys off the result. **Assistant recommendation, not decided:** anchor all three to the result landing, on the grounds the offer-window doc already uses to delete standalone membership -- before a result exists there is no trend, no check-in loop and no retest to be entitled to, so an included month starting at purchase is 30 days of the same empty room, given away rather than sold, and it can expire before the result exists if the kit sits in a drawer. It also gives the retest its clinically correct anchor (90 days after the baseline, not after a card was charged) and removes the ask from the moment a result may route to a GP, which closes stage 9's open GP rail as a side effect. ✅ **DECIDED 2026-09-07 (Keith): THE MEMBERSHIP AUTO-RENEWS ON DAY 31.** Doc: `2026-09-07-auto-renew-at-day-30.md`. Keith: *"I don't see why an opt-in is necessary, as we are sticking to our cadence that the first month is free, and then on day 31, we charge them."* **The assistant's OPT-IN recommendation was wrong and is recorded as an error rather than quietly dropped:** it collapsed **prominence** into **opt-in**, which are different obligations. Numan's ASA ruling (A22-1153049) was for **burying** the subscription, not for having one, and the adopted thesis already states the rule correctly as *"prominent at the point of sale rather than in the T&Cs"*. **What ships instead is a price line on ALL kit pages, NEW SITE ONLY** (`redesign/direction-f`, not the live site): *"Includes 30 days of membership. GBP 47/month after. Cancel anytime."* A second correction inside the same thread: Keith objected that free-trial buy buttons do not carry disclosure statements, which is right about the phrase and wrong about the pattern (Spotify, Audible and Amazon Prime all carry exactly this line at the CTA); it reads as price copy, not as a legal block. **Our case is HARDER than any of those**, because our button says "Order the kit: GBP 99" and the customer has no subscription frame at all, where a Spotify buyer already does. ✅ **DECIDED 2026-09-11 (Keith): THE THREE `/lp/` KIT LANDING PAGES CARRY THE LINE**, closing the last open item in that ruling. Doc: `2026-09-11-lp-kit-pages-carry-the-subscription-line.md`. Built the same day on all seven surfaces from one shared source (`lib/membership/disclosure.ts`), behind `MEMBERSHIP_ENABLED`. 🔴 **BUILDING IT FOUND THAT THE DECIDED HALF HAD NEVER BEEN BUILT**: the line was ruled onto the four `/kits/` routes on 2026-09-07 and existed nowhere in the app except as a private constant inside `/membership`, which 404s with the flag off. 🔴 **AND THIRTEEN SENTENCES ACROSS SEVEN SURFACES STILL ASSERT A ONE-OFF PURCHASE WITH NO SUBSCRIPTION, ONE OF THEM CA-026 C1** (`/kits`, verbatim in the inverted panel: *"no subscription unless you choose one"*). Under the ruling nobody chooses one. None was rewritten, because approved copy is not a rebuild’s to change; `verify-subscription-claims.js` now fails the build whenever the flag is on while any remains, so the flag cannot be turned on until the copy is swept. **Owed: a rewrite from Keith, a pre-flight, and a fresh CA record for C1** — one decision, not three, since it is the same fact on nine surfaces including the homepage line at register row 12a. 🔴 **THE REAL RISK IS THE MERGE, NOT THE RULING.** The live site is safe today only because it says nothing about membership; the moment the redesign merges, the kit page would promise an included month while `MEMBERSHIP_ENABLED` is off, checkout has **no `trial_period_days` anywhere in the repo**, and `createMembership` charges from day one. **The line must render behind `MEMBERSHIP_ENABLED`, or ship with the trial mechanic.** Easier than opt-in would have been: Stripe does this natively and `ACTIVE_MEMBER_STATUSES` **already includes `trialing`**, so only checkout is missing. 🔴 **A COMPLIANCE GAP FELL OUT AND IT IS A LAUNCH GATE: the UK subscription-contracts regime (DMCC Act 2024 Part 4) appears NOWHERE in `03_compliance`**, which handles supplement subscriptions under the CCRs and has **no membership section at all**. It does not ban auto-renew; it attaches prominence, reminder, cooling-off and easy-cancellation duties to exactly the free-trial-converts-to-paid shape just adopted. Doc: `../03_compliance/2026-09-07-dmcca-subscription-regime-gap.md`, written as seven solicitor questions with explicit confidence markers. **Commencement is NOT asserted** and the assistant's knowledge cutoff sits on the wrong side of the expected window, which is why it is a solicitor question and not a search. Also flagged and unresolved: every kit buyer becoming a member turns the metric from attach rate into day-30 retention, and the forecasts should be read accordingly; and £47 was adopted for VAT-threshold stability, which a free first month moves. Earlier: 🔴 **THE MONITORING THESIS'S FALSIFIER_Last updated: 2026-08-30 (🔴 **THE MONITORING THESIS'S FALSIFIER IS TESTABLE TODAY AND NOBODY HAS TESTED IT, BECAUSE THE INSTRUMENT IS LIVE AND THE TRAFFIC IS NOT.** Section 8 said the quiz motive question *"has never been fielded"* and needed an option adding. Checked against the code 2026-08-30: **it is live as quiz step 1** (`TestSelectorQuiz.tsx`) and **the thesis option is already there** as option B, *"I am knackered, foggy, or just do not feel like myself anymore"*. The Van Westendorp block is live too, at step 4. Only *"prepay a bundle or subscribe?"* is genuinely un-fielded. **What is missing is respondents:** live query today returns `qualifier_responses` **0 rows**, **3 users all time, 0 in 30 days**, 1 lab result. ~100 to 200 answers would make the Q1 split mean anything. 🔴 **Second gap, a build task: the answer is not stored queryably** — the route emits a Customer.io `quiz_complete` event and writes no database row, so the number that decides GBP 47 versus GBP 19 to 29 will not be readable by SQL. A dated correction is appended to thesis section 8; the original text is preserved. **Status of the thesis is unchanged: still unproven, still resting on inference, not measurement.** Earlier the same day: ✅ **THE BRAND LEAD IS RULED AND SWEPT: A2, CONFLICT-FREE STAYS THE LEAD AND THE RECORD BECOMES ITS PROOF; THE CATEGORY NOUN IS "A UK MEN'S HEALTH COMPANY".** Keith, 2026-08-30, on `2026-08-30-brand-lead-after-the-monitoring-thesis.md`. Swept the same day across `02_brand/messaging-framework.md`, `02_brand/CONTEXT.md`, `07_sales/funnel/site-funnel-model.md` §1a, this thesis's §7 row and both STATE files. **The pillar moves from "privacy receipt" to PROOF LAYER**: permitted in customer copy as demonstration, still never the opening claim, and "the app is what you market" is explicitly not licence to lead on the app. **No CA-026 re-clearance triggered.** 🔴 **One item stays owed and it is Keith's: the `brand-description.md` rewrite**, two leads stale, feeding affiliate briefs, press and the About page. Earlier the same day: the ruling was drafted because two binding docs contradicted each other, `2026-08-30-brand-lead-after-the-monitoring-thesis.md` is drafted for him to rule on: `02_brand/messaging-framework.md` (2026-07-22) demotes the patient-owned-data pillar and says **do not open customer copy on it**, while `07_sales/funnel/site-funnel-model.md` v2 says **the app is what you market**. They are the same pillar. Verified by git: **the 2026-08-25 monitoring-thesis sweep never touched the brand workspace**, so both brand files still date from the July conflict-free sweep and `brand-description.md` is now two leads stale with its July rewrite still owed. Recommendation in the doc is **A2: conflict-free stays the lead, the record becomes its proof**; the app is evidence for the position, not the position. Also recommends declining "men's wellness brand" as a self-description: wellness is a Phase 0 regulatory tier, not an identity, and pillar 4 already bans the register. **Nothing is adopted and nothing sweeps until section 6 is filled in.** Earlier: 🟢 **THE REVENUE MODEL IS SETTLED: BOTH, WITH A 30-DAY OFFER WINDOW.** Decision doc: `2026-08-26-membership-offer-window.md` (Keith, 2026-08-26). Kits are sold one-off at **full retail, always, to everyone**; membership is a recurring layer on top. Neither is optional: kit-only LTV (~£88) does not cover paid CAC (£193 to £654), and the year-one forecast still has 63% of revenue coming from kits. **A membership may be JOINED only while a lab result has come back within the last 30 days.** Miss it and the way back in is another kit at full retail, which opens a new 30 days; nothing carries over on rejoining. One rule covers four cases (new customer, decliner returning, cancelled member rejoining, what carries over), which is why it is one predicate and not four. It gates JOINING, never STAYING. **Kits carry no member discount**: member pricing is for SUPPLEMENTS, which is what the adopted thesis always said, and it stays dark until supplements are listed in the shop. **Membership can no longer be bought standalone** (the 'Before your first result' screen is deleted): it sells a dated retest and an interpretation of a number, and both are meaningless without a baseline. Built and pushed the same day, commit `c75dcc8`, behind `MEMBERSHIP_ENABLED`. 🔴 **STILL OPEN and named in the decision doc's section 6** [✅ the first-month item was RULED 2026-08-27 and swept 2026-09-06, see the ADOPTED section below; the other items in this list stand]: whether the first month of membership is included in the kit price (BOTH financial models assume it is, the build charges £47 from signup, and there is no trial mechanic — a real model-versus-build divergence); kit price under the membership model (gap-analysis decision #5); £47 versus £19 to £29; and the compliance read plus the membership terms, both still hard launch gates. Earlier: 2026-08-24 (**THE OFFER IS BEING RESHAPED AROUND AN APP, AND THE MECHANISM FOR IT IS ALREADY BUILT AND SWITCHED OFF.** Keith, 2026-08-24, exploratory: kit as gateway, **the app as the product**, supplements demoted to a shop at member price. Positioning is **"one test is not a decision"**: the incumbents advertise the clinical solution and their funnel depends on one result being enough to start a programme, so they cannot occupy that ground. 🔴 **The Confirmation bundle in `lib/bundles/config.ts` already implements it** — second testosterone kit auto-dispatched when the first reads under 12 nmol/L, `CONFIRMATION_INTERVAL_DAYS = 0`, **Ewa signed the immediate recheck 2026-07-26**, all-clear banks for a 6-month recheck — gated off behind `BUNDLES_ENABLED`. **This answers the 2026-07-20 refutation rather than overruling it**: that review assessed a FEATURE (the results brain, which Thriva has) and this is a POSITION (decision support, which nobody occupies). **Two compliance constraints**: the 12 nmol/L figure is BSSM's and must carry its citation inline (already a recorded ASA substantiation exposure), and a sub-12 result routes to a GP per the 2026-06-04 decision, which strengthens rather than blocks the story — we supply the second morning sample a GP needs, we do not diagnose. **Measured the same day**: 3 kit orders ever, 3 users, 0 in 30 days, 17 ranked keywords, best position 18, ~20 organic visits/month, none commercial-intent (`2026-08-24-can-we-sell-four-kits-a-month.md`). **Keith's reading, accepted: that is the absence of marketing, not its failure** — nothing has been promoted. Financial models filed: `financial-model/2026-08-24-membership-90-day-model.md` and `...-year-1-forecast.md` (base case GBP 55,362 revenue / GBP 31,297 profit / 57%, cash-positive from month 2). **NOTHING DECIDED, NOTHING BUILT, NO FLAG FLIPPED.** **Ordering, and it is load-bearing: the app-led-versus-shop-led decision must be made BEFORE any Nutribl stock order and before label artwork is commissioned.** Stock is about GBP 97 and reversible; artwork is where the commercial model becomes physical, and a bottle designed for shelf comparison is a different object from one designed as part of a protocol. The shop-front build in `../09_website-app/docs/2026-08-23-supplement-shop-front-spec.md` also waits on it. If app-led is taken it partially reverses the 2026-08-22 "supplements lead the funnel" decision (supplements stay the MARKETING entry, the kit becomes the first PURCHASE) and needs a decision sweep. Earlier: **SUPPLEMENTS LEAD THE FUNNEL, AND THE LOOP LIVES IN MARKETING** — Keith, 2026-08-22. Three linked decisions, none of them previously written down.)_

---

## 🟢 ADOPTED: the first 30 days of membership are included in the kit price (Keith, 2026-08-27)

**Decision doc: `2026-08-27-first-month-included-in-kit-price.md`. Ruled 2026-08-27, swept 2026-09-06.**

**The ruling.** A kit buyer is a member from the moment the kit is bought. Stage 9 stops being
*"will you start paying £47"* and becomes *"you are already a member until [date], do you want to
carry on"*, which turns the stage-9 emails from a sales sequence into a continuation sequence.

🔴 **Why this needed a sweep at all, and it is the stale-doc pattern the root CLAUDE.md warns about.**
The ruling was propagated into `../08_customer-journey/journey-spine.md` stage 9 and
`../07_sales/funnel/site-funnel-model.md` §4 on the day it was made (commit `d695683`), and **never
recorded as a decision doc of its own**. So `ls 01_strategy/*.md` showed seven decision docs and not
this one, this file went on listing the question as STILL OPEN for ten days, and
`2026-08-26-membership-offer-window.md` §6 went on calling it *"still divergent, still undecided"*.
Every file was internally consistent; only a reader who opened all four could see the disagreement.
Keith caught it when the assistant quoted the stale text back to him.

**It resolves a model-versus-build divergence in favour of the models, so no forecast moves.** Both
`financial-model/2026-08-24-membership-90-day-model.md` and `...-year-1-forecast.md` already assumed
the included month. **The build is the side now out of step.**

🔴 **THE BUILD DOES NOT IMPLEMENT THIS, AND IT IS A LAUNCH BLOCKER.** Flagged, not changed (sweep
invariant 4). There is **no `trial_period_days` or `trial_end` anywhere in the repo**, so checkout has
no mechanic that can express an included month; `createMembership` stamps `started_at` and
`next_retest_due_at` from the checkout moment; `lib/subscriptions/products.ts` carries `£47/mo` with no
notion of an included period. **The useful half:** `ACTIVE_MEMBER_STATUSES` in
`lib/membership/entitlement.ts` **already includes `trialing`**, kept in step with the partial unique
index `memberships_one_live_per_user`, so an included month lights up an existing tested path rather
than needing a new one. What is missing is only the writer. **Not customer-visible**: membership is
behind `MEMBERSHIP_ENABLED`, `/membership` `notFound()`s when the flag is off, and no public page
quotes the price.

🔵 **THREE THINGS FELL OUT OF THIS. ONE IS NOW DECIDED, TWO ARE STILL KEITH'S.**
1. ✅ **DECIDED 2026-09-07: the RESULT LANDING** (Keith, *"anchor everything to results"*).
   Doc: `2026-09-07-anchor-everything-to-the-result.md`. The recommendation below was adopted as
   written. Four of the eight retest mechanisms already complied; the membership and the timed
   bundles move; seq-04 e5 is the one carve-out, because it measures supplement effect and its
   clock legitimately starts at `subscription_started`. 🔴 **It opens one new question, not
   decided: a timed bundle whose customer never posts a sample.** Today the prepaid second kit
   ships at purchase + 90 regardless; anchored to the result it would never ship. Recommendation
   was a purchase + 180 backstop; **revised the same day to purchase + 90, i.e. the timed bundles are EXEMPT**, because the 180 contradicted live approved T&Cs promising the retest "about 90 days after your purchase". No terms change, no Ewa re-approval, no bundle code change. **What was written on 2026-08-27:** Three anchors exist today:
   the included month would
   start at PURCHASE, the offer window starts when the RESULT lands, and `createMembership` uses
   STRIPE CHECKOUT. A slow lab puts them out of step. *Recommendation, not decided:* anchor all three
   to the result landing. Before a result exists there is no trend, no check-in loop and no retest to
   be entitled to, so an included month starting at purchase is 30 days of an empty room, and it can
   expire before the result exists if the kit sits in a drawer. It also gives the retest its
   clinically correct anchor.
2. **Auto-renew versus opt-in at day 30.** *Recommendation, not decided:* OPT-IN, on brand grounds
   rather than legal ones. A card charging on day 31 for something the kit page called "included" is
   the pattern the conflict-free position forbids. It costs conversion and that is the trade.
3. **What it does to £47**, which was adopted for VAT-threshold stability. A free first month moves
   that arithmetic. Needs the accountant.

Also flagged: every kit buyer becoming a member turns the headline metric from **attach rate** into
**day-30 retention**, and the forecasts should be read accordingly.

✅ **Swept 2026-09-06.** CREATED: `2026-08-27-first-month-included-in-kit-price.md`. UPDATED: this file,
`financial-model/2026-08-24-membership-90-day-model.md` (assumption becomes adopted),
`...-membership-year-1-forecast.md` (same), `../10_launch-ops/2026-08-24-supplement-membership-gap-analysis.md`
(dated note on the decision table). FORWARD-POINTER: `2026-08-26-membership-offer-window.md` §6, body
unchanged per sweep invariant 1. VERIFIED ALREADY CORRECT: `../07_sales/funnel/site-funnel-model.md`,
`../07_sales/STATE.md`, `../08_customer-journey/journey-spine.md`. **Clean negatives, recorded so they
are not re-asked:** `../04_products/CONTEXT.md` and `../10_launch-ops/STATE.md` say nothing about the
membership; no public page or email template quotes the membership price, so **no approved copy moves
and there is no Ewa escalation**; no published artefact carries the fact.

---

## ✅ ADOPTED: conflict-free stays the lead, the record becomes its proof (Keith, 2026-08-30)

**Doc: `2026-08-30-brand-lead-after-the-monitoring-thesis.md`. RULED A2 + "men's health company" on 2026-08-30 and swept the same day.**
Raised by Keith on 2026-08-29: *"our brand remains the same, but it doesn't remain the same."*

**The contradiction.** `../02_brand/messaging-framework.md` demoted the Patient-owned data pillar on
2026-07-22 with the instruction *"do not open customer copy on them"*. `../07_sales/funnel/site-funnel-model.md`
v2 (2026-08-27) says *"the app is what you market"*. **Those are the same pillar**, and pillar 1 of the
messaging framework, written May 2026, already reads as the monitoring thesis in full. So the August
work re-promoted what July demoted and nobody recorded the reversal. Copy written today can satisfy
either doc and contradict the other.

🔴 **Verified cause, checked against git rather than assumed: the 2026-08-25 sweep never reached the
brand workspace.** Both `messaging-framework.md` and `brand-description.md` were last touched by
commit `647e91b`, the 2026-07-22 conflict-free sweep. The recommendation in the doc is that the brand
workspace becomes an explicit stop on any sweep that changes what we sell or what we lead on.

**Second finding: `brand-description.md` is two leads stale.** It still leads on the May
patient-owned-data framing, carries a 2026-07-22 banner saying the conflict-free rewrite is owed and
undone, and predates the August thesis entirely. It is the source for affiliate briefs, press, the
About page, the homepage meta description and every social bio.

**Two questions are put to Keith.** (A) What leads: conflict-free, or the record. (B) The category
noun: "men's health company" or something else. They are separable.

**Recommendation: A2, and keep "men's health company."** Conflict-free is the only position the July
review found structurally uncopyable, and the record becomes its proof rather than its headline. The
demo makes the case not because it is an app but because of what it visibly refuses to do, and those
refusals are what a vertical-committed rival cannot copy. "Men's wellness brand" is declined because
wellness is the Phase 0 regulatory tier, not an identity.

✅ **Swept 2026-08-30.** UPDATED: `../02_brand/messaging-framework.md` (governing banner amended, pillar 1
re-titled and given its proof-layer role), `../02_brand/CONTEXT.md` (both the pillar list and the LEAD SUPERSEDED
banner, plus the category noun), `../07_sales/funnel/site-funnel-model.md` §1a (the qualifier on "the app is what
you market"), `2026-08-24-vertical-agnostic-monitoring-thesis.md` §7, and both STATE files. A dated forward-pointer
was added to `2026-07-22-conflict-free-positioning-decision.md`; its body is unchanged.

**Clean negatives, recorded so they are not re-asked.** No file in `.claude/skills/` states the brand lead. Neither
does `../06_marketing/content-machine/CONTEXT.md`, so the canonical-asset chain inherits the lead through
`messaging-framework.md` and needed no edit. **`Pillar F, patient-owned data` in `../06_marketing/seo-ai-search/` is
a CONTENT pillar, not the brand lead pillar** — same words, different taxonomy, deliberately left alone. **No
executable carrier exists**: no constraint, enum, validator or scanner encodes a brand lead.

🔴 **ESCALATED, owner Keith: the `brand-description.md` rewrite.** Its banner is re-stamped and points here; the
long / short / one-liner copy was deliberately not written by the sweep. Needs `/compliance-preflight` before
external reuse. **No Ewa escalation**: under A2 no approved claim moves, so CA-026 is unaffected.
