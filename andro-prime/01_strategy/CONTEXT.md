...
├── 2026-07-22-conflict-free-positioning-decision.md ← ADOPTED: conflict-free positioning is THE brand position (category-wide, binding on all copy)
├── trt-launch-readiness-2026-05-08.md
├── ltv-cac-profitability-model-2026-06-26.md    ← LTV:CAC model (annotated 2026-06-27)
├── ltv-cac-profitability-model-2026-07-21.md    ← Bundle-aware LTV:CAC model (extends 06-26 with the two-kit bundle SKUs)
...
│   └── 2026-07-20-uk-testing-competitor-teardown.md ← July teardown; seeded the conflict-free position
├── entity-structure/
│   ├── 2026-05-12-single-entity-decision.md     ← Single-entity Andro Prime Ltd (50/50)
│   └── shareholders-agreement-draft.md
├── financial-model/                             ← Phase 0 + tiered models (phase0-financial-model-v1 [Gate 0A's ~£5,950 cap derives from it], option-4, tiered-platform v1/v2, SEO forecast; .md + .xlsx + build scripts)
├── research/                                    ← 2026-05-08 research (demand, competitive, reddit signal, cash-target, funnel math) + a July 2026 wave (reddit-quora + vitall unmet-needs, fable adversarial review, mainstream-buyer deep research, vitall strategy report)
...
| Brand position | **Conflict-free men's health testing** (ADOPTED 2026-07-22, category-wide, binding on all copy). Rail: **never write "we will never sell testosterone"** (the post-CQC clinic plans TRT); the durable claim is separation of incentives (testing and treatment separated; we earn the same whether a result is low or fine). See `2026-07-22-conflict-free-positioning-decision.md`. |
...
| Paid acquisition | Cold paid **never pays back** (best modelled cell £207 → 1.03:1 with bundle AOV, still well under the ~3:1 healthy ratio; per the 2026-07-21 bundle-aware model). Only owned (~£0 CAC) + affiliate (~£30) clear the ~£29 healthy-CAC bar. Attach rate + tenure are first-order LTV levers; price is second-order. |
...
- `/decision-sweep`: propagate a landed strategic decision (pricing, routing, positioning, entity) through every affected doc across workspaces.
- `/wrap`, `/task-observer`: session close-out (STATE/ClickUp/git) and skill-friction capture.
- `competitor-alternatives`, `customer-research`: competitor teardowns and demand/VOC synthesis feeding `competitive-landscape/` and `research/`.
- `pricing-strategy`: pricing and packaging logic for the financial model.
- `dataviz`: charts for the financial model and the LTV/CAC work.

**MCPs & tools:**

- **dataforseo** (MCP, wired): competitor teardowns, keyword/demand sizing, SERP and trends research (the `research/` and `competitive-landscape/` work).
- **reddit** (MCP, connector): demand and VOC signal mining (the reddit/quora unmet-needs research).
- **graphify** (MCP, wired): repo and doc discovery across workspaces; default over grep.
- **Google Drive / gws CLI** (connector / local CLI): financial-model spreadsheets and strategy docs.
